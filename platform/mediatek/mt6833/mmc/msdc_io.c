...
void update_mmc_status(void *fdt)
{
    ...
    node = fdt_node_offset_by_compatible(fdt, -1, "mediatek,mt6779-mmc");
    if (node < 0) {
...
#if defined(MTK_MSDC_BRINGUP_DEBUG)
    if (id == 0) {
        dprintf(ALWAYS, "MSDC0 MODE8[%x] = 0x%X\tshould: 0x1111111?\n",
            MSDC0_GPIO_MODE8, MSDC_READ32(MSDC0_GPIO_MODE8));
        dprintf(ALWAYS, "MSDC0 MODE9[%x] = 0x%X\tshould: 0x???11111\n",
            MSDC0_GPIO_MODE9, MSDC_READ32(MSDC0_GPIO_MODE9));
        dprintf(ALWAYS, "MSDC0 IES   [%x] = 0x%X\tshould: 0x???1FFE?\n",
            MSDC0_GPIO_IES, MSDC_READ32(MSDC0_GPIO_IES_ADDR));
        dprintf(ALWAYS, "MSDC0 SMT   [%x] = 0x%X\tshould: 0x????3FFC\n",
            MSDC0_GPIO_SMT, MSDC_READ32(MSDC0_GPIO_SMT_ADDR));
        dprintf(ALWAYS, "MSDC0 TDSEL0[%x] = 0x%X,[%x] = 0x%X
            should: 0x000000?? should: 0x??000000\n",
            MSDC0_GPIO_TDSEL0,
            MSDC_READ32(MSDC0_GPIO_TDSEL0),
            MSDC0_GPIO_TDSEL1,
            MSDC_READ32(MSDC0_GPIO_TDSEL1));
        dprintf(ALWAYS, "MSDC0 RDSEL0[%x] = 0x%X,[%x] = 0x%X,[%x] = 0x%X
            should: 0x?000000? should: 0x(0b??00)0000000 should: 0x??\?(0b??00)0000\n",
            MSDC0_GPIO_RDSEL0,
        ...
        dprintf(ALWAYS, "MSDC0 DRV0  [%x] = 0x%X,[%x] = 0x%X
            should: 0x(0b??00)92492(0b01?\?)? should:?????249\n",
            MSDC0_GPIO_DRV0,
...
#ifndef FPGA_PLATFORM
    if (id == 0)
        MSDC_SET_FIELD(MSDC0_GPIO_SMT, MSDC0_SMT_ALL_MASK, (set_smt ? 0x7FF : 0));
    else if (id == 1)
...
void msdc_set_driving_by_id(u32 id, struct msdc_cust *msdc_cap)
{
    ...
    if (id == 0) {
        ...
        MSDC_SET_FIELD(MSDC0_GPIO_DRV1, MSDC0_DRV1_DAT6_MASK, msdc_cap->dat_drv);
        MSDC_SET_FIELD(MSDC0_GPIO_DRV1, MSDC0_DRV1_DAT7_MASK, msdc_cap->dat_drv);
