...
#if DBG_USB_QMU
#define DBG_QMU(x...)        dprintf(INFO, x)
#else
...
void udc_descriptor_free_all(void)
{
    while (desc_list != NULL)  {
        struct udc_descriptor *desc = desc_list;
...
void qmu_tx_interrupt(u8 ep_num)
{
    ...
    DBG_QMU("tx_gpd_last 0x%x, gpd_current 0x%x, gpd_end 0x%x\n",
    ...
    while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
        ...
        DBG_QMU("Tx gpd %x info { HWO %d, BPD %d, Next_GPD %x , DataBuffer %x,
    ...
    DBG_QMU("tx_gpd_last[%d]: 0x%x\n", ep_num, (u32)tx_gpd_last[ep_num]);
    DBG_QMU("tx_gpd_end[%d]:  0x%x\n", ep_num, (u32)tx_gpd_end[ep_num]);
    DBG_QMU("Tx %d complete\n", ep_num);
    return;
}
...
void qmu_rx_interrupt(u8 ep_num)
{
    ...
    DBG_QMU("ep_num: %d ,Rx_gpd_last: 0x%x, gpd_current: 0x%x, gpd_end: 0x%x\n",
    ...
    while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
        ...
        DBG_QMU("Rx gpd info { HWO %d, Next_GPD %x ,DataBufferLength %d,
