...
void revise_part_name(const char *part_name, char *revised_part_name,
        int revised_part_name_size, const char *suffix)
{
    strlcpy(revised_part_name, part_name, revised_part_name_size);

#ifdef MTK_WITH_COMMON_BOOTCTRL
    int i;
    const char *part_list[][2] = { {"lk", "lk"}, {"spmfw", "spmfw"}, {"md1img", "md1img"},
            {"md1dsp", "md1dsp"}, {"medmcu_1", "medmcu"}, {"dpm_1", "dpm"} };
    for (i = 0; i < countof(part_list); i++) {
        if (!strcmp(part_name, part_list[i][0])) {
            strlcpy(revised_part_name, part_list[i][1], revised_part_name_size);
            strlcat(revised_part_name, suffix, revised_part_name_size);
            break;
        }
    }
#endif
