...
int dpmgr_path_switch_layer(void *dp_handle, unsigned int layer, unsigned int en,
                            int encmdq)
{
    ASSERT(dp_handle != NULL);
    struct ddp_path_handle_t *handle = (struct ddp_path_handle_t *)dp_handle;
    ...
    DISP_LOG_V("%s:%d on scenario %s\n", __func__,
                layer, ddp_get_scenario_name(handle->scenario));
    ...
    for (i = 0; i < module_num; i++) {
        module_name = modules[i];
        if (ddp_modules_driver[module_name] != 0) {
            if (ddp_modules_driver[module_name]->switch_layer != 0)
                ddp_modules_driver[module_name]->switch_layer(module_name, layer, en, cmdqHandle);
        }
    }
    return 0;
}
