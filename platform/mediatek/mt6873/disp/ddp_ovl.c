                        ...
                        void *handle)
{
    ...
    unsigned int local_layer = 0, global_layer = 0, regval = 0;
    int idx = ovl_index(module);
...
//        ovl_layer_config(module, local_layer, has_sec_layer, ovl_cfg, handle);
    ...
    regval = DISP_REG_GET(idx * DISP_INDEX_OFFSET + DISP_REG_OVL_SRC_CON);
    enabled_layers |= regval;
    if (enabled_layers != regval) {
        DDPMSG("module:%d OVL enable layer:0x%x, regval:0x%x\n",
            module, enabled_layers, regval);
