...
#define REG_DUMP(addr) dcm_info("%-30s(0x%lx): 0x%08x\n", #addr, addr, reg_read(addr))
...
/* SMC call for mcusys pbi, turbo, stall, and apb initializations */
...
static int dcm_smc_call_control(int onoff, unsigned int mask)
{
    ...
    __smc_conduit(MTK_SIP_BL_DCM_CONTROL, onoff, mask, 0, 0, 0, 0, 0,
                     &smccc_res);
