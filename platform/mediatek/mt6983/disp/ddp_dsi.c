                                 ...
                                 LCM_DSI_PARAMS *dsi_params)
{
    ...
    unsigned int is_cphy = 0;

    if (dsi_params->dsc_enable)
        width = (width + 2) / 3;

    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        is_cphy = _dsi_context[i].dsi_params.IsCphy;
        ...
        dprintf(0, "%s, mode=0x%x, tmp=0x%x, width=%d, height=%d, rw_times=%d\n",
                  __func__, dsi_params->mode, tmp, width, height, rw_times);
...
void DSI_Set_Null_PKT(struct disp_ddp_path_config *config, void *cmdq_handle)
{
    DISP_REG_SET_FIELD(cmdq_handle, DSI_CMD_HFP_BLANKING_NULL_LEN,
        DSI_REG_BASE[0] + DISP_REG_DSI_CMD_TYPE1_HS,
        config->dsi_config.cmd_null_pkt_len);
    DISP_REG_SET_FIELD(cmdq_handle, DSI_CMD_HFP_BLANKING_NULL_EN,
        DSI_REG_BASE[0] + DISP_REG_DSI_CMD_TYPE1_HS, 1);
    DISPMSG("set cmd_null_pkt_len is %d\n", config->dsi_config.cmd_null_pkt_len);
}
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        ...
        if (data_Rate != 0) {
            ...
            if (data_Rate >= 8000) {
                ...
                div3_en      = 0;
                //prediv      = 0;
            } else if (data_Rate >= 1000) { /* 1G ~ 1.5G */
                pcw_ratio = 2;
                posdiv      = 1;
                div3      = 3;
                div3_en      = 1;
                //prediv      = 0;
            } else if (data_Rate >= 750) { /* 750M ~ 1G */
                pcw_ratio = 8;
                posdiv      = 3;
                div3      = 1;
                div3_en      = 0;
                //prediv      = 0;
            } else if (data_Rate >= 500) { /* 500M ~ 750M */
                pcw_ratio = 4;
                posdiv      = 2;
                div3      = 3;
                div3_en      = 1;
                //prediv      = 0;
            } else if (data_Rate >= 450) { /* 450M ~ 500M */
                pcw_ratio = 16;
                posdiv      = 4;
                div3      = 1;
                div3_en      = 0;
...
void DSI_DPHY_clk_setting(enum DISP_MODULE_ENUM module, void *cmdq, LCM_DSI_PARAMS *dsi_params)
{
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        ...
        if (data_Rate > 2000)
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_VOLTAGE_SEL, FLD_RG_DSI_V2I_REF_SEL, 0x4);
        else if (data_Rate > 1200)
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_VOLTAGE_SEL, FLD_RG_DSI_V2I_REF_SEL, 0x2);
        else
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_VOLTAGE_SEL, FLD_RG_DSI_V2I_REF_SEL, 0x0);
        ...
        if (data_Rate != 0) {
            ...
            if (dsi_params->ssc_disable != 1) {
                ...
                ASSERT(delta1 <= 8);
                pdelta1 = data_Rate * pcw_ratio * div3 * 5 / 26 * 262144 / 1000 / 433;
                    ...
                    void *cmdq_handle)
{
   ...
   DSI_PHY_TIMCONFIG(module, NULL, &(config->dsi_config));
    ...
    if (config->dsi_config.mode == CMD_MODE && config->dsi_config.cmd_null_pkt_en)
        DSI_Set_Null_PKT(config, cmdq_handle);
...
int ddp_dsi_start(enum DISP_MODULE_ENUM module, void *cmdq)
{
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        DISP_REG_SET_FIELD(NULL, DSI_SHADOW_DEBUG_FLD_BYPASS_SHADOW,
                            DSI_REG_BASE[i] + DISP_REG_DSI_SHADOW_DEBUG, 1);
        DISP_REG_SET_FIELD(NULL, DSI_SHADOW_DEBUG_FLD_READ_WORKING,
                            DSI_REG_BASE[i] + DISP_REG_DSI_SHADOW_DEBUG, 1);
    }
#endif
    ...
    if (module == DISP_MODULE_DSIDUAL) {
        ...
        DISP_REG_SET_FIELD(NULL, DSI_START_FLD_DSI_START, DSI_REG_BASE[0] + DISP_REG_DSI_START, 0);
        DISP_REG_SET_FIELD(NULL, DSI_START_FLD_DSI_START, DSI_REG_BASE[1] + DISP_REG_DSI_START, 0);
        ...
        if (_dsi_context[0].dsi_params.mode != CMD_MODE) {
            ...
            DISP_REG_SET_FIELD(NULL, DSI_COM_CON_FLD_DSI_DUAL_EN,
                                DSI_REG_BASE[1] + DISP_REG_DSI_COM_CON, 1);
        }

        DISP_REG_SET(NULL, DISP_REG_DSI_INTEN + DSI_REG_BASE[0], 0x0);
        DISP_REG_SET(NULL, DISP_REG_DSI_INTEN + DSI_REG_BASE[1], 0x0);
        DSI_SetMode(module, NULL, _dsi_context[0].dsi_params.mode);
        DSI_clk_HS_mode(module, NULL, true);
    } else if (module == DISP_MODULE_DSI0 || module == DISP_MODULE_DSI1) {
        DISPFUNC();
        i = DSI_MODULE_BEGIN(module);
        DISP_REG_SET(NULL, DISP_REG_DSI_INTEN + DSI_REG_BASE[i], 0x0);
        DSI_SetMode(module, NULL, _dsi_context[i].dsi_params.mode);
        DSI_clk_HS_mode(module, NULL, true);
    }
    return 0;
}
