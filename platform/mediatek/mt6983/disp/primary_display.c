...
int primary_display_config_input(struct disp_input_config *input)
{
    ...
    last_config = dpmgr_path_get_last_config(pgc->dpmgr_handle);
    if (last_config != NULL) {
        memcpy(&data_config, last_config, sizeof(struct disp_ddp_path_config));
    } else {
        DISPERR("dpmgr_path_get_last_config return NULL\n");
        return -1;
    }
