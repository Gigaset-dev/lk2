...
{
    struct boot_tag_display_info *p = (struct boot_tag_display_info *)&tag->data;
    unsigned int i = 0;

    display_info.islcmfound = p->islcmfound;
    for (; i < MTK_LCM_NAME_LENGTH; i++)
        display_info.lcm_name[i] = p->lcm_name[i];
}
...
{
    /* LAB sec control */
    writel(0, SMI_LARB0_SEC_CON + 0 * 0x4);
    writel(0, SMI_LARB0_SEC_CON + 7 * 0x4);
    writel(0, SMI_LARB0_SEC_CON + 8 * 0x4);
    writel(0, SMI_LARB0_SEC_CON + 9 * 0x4);

    writel(0, SMI_LARB20_SEC_CON + 0 * 0x4);
    writel(0, SMI_LARB20_SEC_CON + 7 * 0x4);
    writel(0, SMI_LARB20_SEC_CON + 8 * 0x4);
    writel(0, SMI_LARB20_SEC_CON + 9 * 0x4);

    /* Disable sec display control */
    writel(0xfffffffe, DISP_REG_MMSYS_SECURITY_DISABLE2);
    writel(0xfffffffe, DISP1_REG_MMSYS_SECURITY_DISABLE2);
}
...
void mt_disp_update(u32 x, u32 y, u32 width, u32 height)
{
    ...
    DISPDBG("fb dump: 0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
...
void target_atag_videolfb(void *fdt)
{
    ...
    *p++ = DISP_GetVRamSize();
    ...
    ret = fdt_setprop(fdt, offset, "atag,videolfb", ptr, (void *)p - (void *)ptr);
    if (ret)
        DISPERR("[%s %d] fdt_setprop failed\n", __func__, __LINE__);
}
...
void disp_lcm_fill_lcm_dts_phandle(void *fdt)
{
    ...
    const void *val, *val1;
    char buf[100];
    const char *phy_type = "mediatek,mt6885-mipi-tx-cphy";

#ifdef MTK_FORCE_DISABLE_PRIM_DUAL_PIPE
    nodeoffset = fdt_path_offset(fdt, "/dispsys_config");
    if (nodeoffset < 0)
        nodeoffset = fdt_path_offset(fdt, "/soc/dispsys_config");
    fdt_setprop_empty(fdt, nodeoffset, "force_no_prim_dual_pipe");
#endif

    ret = snprintf(buf, sizeof(buf) - 1, "/soc/dsi");
    if (ret < 0)
        DISPERR("[%s %d] snprintf failed\n", __func__, __LINE__);
    ...
    if (need_prefix)
        ret = snprintf(buf, sizeof(buf) - 1, "/soc/dsi/panel%d/port/endpoint", idx);
    else
        ret = snprintf(buf, sizeof(buf) - 1, "/dsi/panel%d/port/endpoint", idx);
    if (ret < 0)
        DISPERR("[%s %d] snprintf failed\n", __func__, __LINE__);
    ...
    ret = fdt_setprop(fdt, nodeoffset, "remote-endpoint", &phandle, sizeof(phandle));
    if (ret)
        DISPERR("[%s %d] fdt_setprop failed\n", __func__, __LINE__);
...
error:
    ...
    if (dts_is_cphy) {
        ...
        ret = fdt_setprop_string(fdt, nodeoffset1, "compatible", phy_type);
        if (ret)
            DISPERR("[%s %d] fdt_setprop_string failed\n", __func__, __LINE__);
