    ...
    void *handle)
{
    ...
    struct lcm_spr_config_params *spr_params = &dsi->spr_params;
    ...
    /* init_delay_height should be 0, from designer info */
    init_delay_height = 0;
    ...
    if (dsi->spr_enable && spr_params->relay == 0) {
        val = ((spr_params->relay == 0) ?
            0x1 : 0) << 26;
        switch (spr_params->spr_format_type) {
        case MTK_PANEL_RGBG_BGRG_TYPE:
            val |= 0x00e10d2;
            enc_slice_width = (params->slice_width + 1) / 2;
            break;
        ...
        case MTK_PANEL_RGBRGB_BGRBGR_TYPE:
            val |= 0x1924186;
            enc_slice_width = (params->slice_width * 2 + 2) / 3;
            break;
        ...
        DISP_REG_SET(handle, base + DISP_REG_DSC_SPR, val);
        params->chunk_size = (enc_slice_width*params->bit_per_pixel/16 + 7)/8;
    } else {
    ...
    if (dsi->spr_enable && spr_params->relay == 0) {
        pic_group_width_m1 = (enc_pic_width + 2) / 3 - 1;
        DISP_REG_SET(handle, base + DISP_REG_DSC_PIC_W,
            (pic_group_width_m1 << 16) + width);
        slice_group_width_m1 = (enc_slice_width + 2) / 3 - 1;
        DISP_REG_SET(handle, base + DISP_REG_DSC_SLICE_W,
            (slice_group_width_m1 << 16) + params->slice_width);
    }
    ...
    if (dsi->spr_enable && spr_params->relay == 0) {
        DISP_REG_SET(handle, base + DISP_REG_DSC_CFG, 0x0001d822);
        DISP_REG_SET(handle, base + DISP_REG_DSC_PPS19, 0xd16a);
        DISP_REG_SET(handle, base + DISP_REG_DSC_PPS18, 0xd147d125);
        DISP_REG_SET(handle, base + DISP_REG_DSC_PPS17, 0xd125d925);
        DISP_REG_SET(handle, base + DISP_REG_DSC_PPS16, 0xd923e103);
        DISP_REG_SET(handle, base + DISP_REG_DSC_PPS12, 0x800840);
    }
