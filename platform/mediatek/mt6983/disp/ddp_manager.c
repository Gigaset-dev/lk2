...
static int _dpmgr_path_connect(enum DDP_SCENARIO_ENUM scenario, void *handle)
{
    ...
    for (k = 0; k <= has_dual_path; k++) {
        ...
        for (i = 0; i < module_num; i++) {
            module = modules[i];
            mod = (unsigned int)module;
            if (ddp_modules_driver[mod] &&
                ddp_modules_driver[mod]->connect) {
                int prev = i == 0 ? DISP_MODULE_UNKNOWN : modules[i - 1];
                int next = i == module_num - 1 ? DISP_MODULE_UNKNOWN : modules[i + 1];

                ddp_modules_driver[mod]->connect(module, prev, next, 1, handle);
            }
        }
    }
...
void *dpmgr_create_path(enum DDP_SCENARIO_ENUM scenario, void *cmdq_handle)
{
    ...
    if (path_handle != NULL) {
        ...
        for (i = 0; i < module_num; i++) {
            ...
            mod_name = (unsigned int)module_name;
            content->module_usage_table[mod_name]++;
            content->module_path_table[mod_name] = path_handle;
        }
        content->handle_cnt++;
...
int dpmgr_destroy_path(void *dp_handle)
{
    ...
    if (handle != NULL) {
        ...
        for (i = 0; i < module_num; i++) {
            ...
            mod_name = (unsigned int)module_name;
            content->module_usage_table[mod_name]--;
            content->module_path_table[mod_name] = NULL;
        }
        content->handle_cnt--;
...
int dpmgr_path_set_dst_module(void *dp_handle, enum DISP_MODULE_ENUM dst_module)
{
    ...
    ASSERT(handle->scenario < DDP_SCENARIO_MAX);
...
int  dpmgr_path_init(void *dp_handle, int encmdq)
{
    ...
    for (k = 0; k <= has_dual_path; k++) {
        ...
        for (i = 0; i < module_num; i++) {
            module_name = modules[i];
            mod_name = (unsigned int)module_name;
            if (ddp_modules_driver[mod_name] != 0) {
                if (ddp_modules_driver[mod_name]->init != 0)
                    ddp_modules_driver[mod_name]->init(module_name, cmdqHandle);

                if (ddp_modules_driver[mod_name]->set_listener != 0)
                    ddp_modules_driver[mod_name]->set_listener(module_name,
                                                                dpmgr_module_notify);
            }
        }
    }
...
int  dpmgr_path_deinit(void *dp_handle, int encmdq)
{
    ...
    if (ddp_path_is_dual(handle->scenario))
        ddp1_mutex_clear(handle->hwmutexid, cmdqHandle);
    ...
    for (i = 0; i < module_num; i++) {
        ...
        mod_name = (unsigned int)module_name;
        if (ddp_modules_driver[mod_name] != 0) {
            if (ddp_modules_driver[mod_name]->deinit != 0) {
                DISP_LOG_V("scenario %s deinit module  %s\n",
                           ddp_get_scenario_name(handle->scenario),
                           ddp_get_module_name(module_name));
                ddp_modules_driver[mod_name]->deinit(module_name, cmdqHandle);
            }
            if (ddp_modules_driver[mod_name]->set_listener != 0)
                ddp_modules_driver[mod_name]->set_listener(module_name, NULL);
        }
    }
    handle->power_sate = 0;
...
int dpmgr_path_start(void *dp_handle, int encmdq)
{
    ...
    for (k = 0; k <= has_dual_path; k++) {
        ...
        for (i = 0; i < module_num; i++) {
            ...
            mod_name = (unsigned int)module_name;
            if (ddp_modules_driver[mod_name] != 0) {
                if (ddp_modules_driver[mod_name]->start != 0)
                    ddp_modules_driver[mod_name]->start(module_name, cmdqHandle);
            }
        }
    }
    return 0;
...
int dpmgr_path_stop(void *dp_handle, int encmdq)
{
    ...
    for (i = 0; i < module_num; i++) {
        ...
        mod_name = (unsigned int)module_name;
        if (ddp_modules_driver[mod_name] != 0) {
            if (ddp_modules_driver[mod_name]->stop != 0) {
                DISP_LOG_V("scenario %s  stop module %s\n", ddp_get_scenario_name(handle->scenario),
                           ddp_get_module_name(module_name));
                ddp_modules_driver[mod_name]->stop(module_name, cmdqHandle);
            }
        }
    }
    return 0;
...
int dpmgr_path_reset(void *dp_handle, int encmdq)
{
    ...
    for (i = 0; i < module_num; i++) {
        ...
        mod_name = (unsigned int)module_name;
        if (ddp_modules_driver[mod_name] != 0) {
            if (ddp_modules_driver[mod_name]->reset != 0) {
                DISP_LOG_V("scenario %s  reset module %s\n",
                           ddp_get_scenario_name(handle->scenario),
                           ddp_get_module_name(module_name));
                ddp_modules_driver[mod_name]->reset(module_name, cmdqHandle);
            }
        }
    }
    return 0;
...
int dpmgr_path_config(void *dp_handle, struct disp_ddp_path_config *config, int encmdq)
{
    ...
    for (k = 0; k <= has_dual_path; k++) {
        ...
        for (i = 0; i < module_num; i++) {
            ...
            mod_name = (unsigned int)module_name;
            if (ddp_modules_driver[mod_name] != 0) {
                if (ddp_modules_driver[mod_name]->config != 0)
                    ddp_modules_driver[mod_name]->config(module_name, config,
                                                                cmdqHandle);
                ...
                if (dpmgr_is_PQ(module_name) == 1) {
                    if (ddp_modules_driver[mod_name]->bypass != NULL)
                        ddp_modules_driver[mod_name]->bypass(module_name, 1);
                }
            }
        }
    }
                            ...
                            enum CMDQ_STATE state)
{
    ...
    for (i = 0; i < module_num; i++) {
        ...
        mod_name = (unsigned int)module_name;
        if (ddp_modules_driver[mod_name] != 0) {
            if (ddp_modules_driver[mod_name]->build_cmdq != 0) {
                DISP_LOG_D("%s build cmdq, state=%d\n", ddp_get_module_name(module_name), state);
                ddp_modules_driver[mod_name]->build_cmdq(module_name,
...
int dpmgr_path_trigger(void *dp_handle, void *trigger_loop_handle, int encmdq)
{
    ...
    for (i = 0; i < module_num; i++) {
        ...
        mod_name = (unsigned int)module_name;
        if (ddp_modules_driver[mod_name] != 0) {
            if (ddp_modules_driver[mod_name]->trigger != 0)
                ddp_modules_driver[mod_name]->trigger(module_name,
                                                            trigger_loop_handle);
        }
    }
    return 0;
...
int dpmgr_path_power_off(void *dp_handle, enum CMDQ_SWITCH encmdq)
{
    ...
    for (i = 0; i < module_num; i++) {
        ...
        mod_name = (unsigned int)module_name;
        if (ddp_modules_driver[mod_name] != 0) {
            if (ddp_modules_driver[mod_name]->power_off != 0) {
                DISP_LOG_I(" %s power off\n", ddp_get_module_name(module_name));
                ddp_modules_driver[mod_name]->power_off(module_name,
...
int dpmgr_path_power_on(void *dp_handle, enum CMDQ_SWITCH encmdq)
{
    ...
    for (i = 0; i < module_num; i++) {
        ...
        mod_name = (unsigned int)module_name;
        if (ddp_modules_driver[mod_name] != 0) {
            if (ddp_modules_driver[mod_name]->power_on != 0) {
                DISP_LOG_I("%s power on\n", ddp_get_module_name(module_name));
                ddp_modules_driver[mod_name]->power_on(module_name,
                                                            encmdq?handle->cmdqhandle:NULL);
            }
        }
    }
...
int dpmgr_path_is_busy(void *dp_handle)
{
    ...
    for (i = module_num-1; i >= 0; i--) {
        ...
        mod_name = (unsigned int)module_name;
        if (ddp_modules_driver[mod_name] != 0) {
            if (ddp_modules_driver[mod_name]->is_busy != 0) {
                if (ddp_modules_driver[mod_name]->is_busy(module_name)) {
...
int dpmgr_set_lcm_utils(void *dp_handle, void *lcm_drv, void *fdt)
{
    ...
    for (i = 0; i < module_num; i++) {
        ...
        mod_name = (unsigned int)module_name;
        if (ddp_modules_driver[mod_name] != 0) {
            if ((ddp_modules_driver[mod_name]->set_lcm_utils != 0) && lcm_drv) {
                DISP_LOG_I("%s set lcm utils\n", ddp_get_module_name(module_name));
                ddp_modules_driver[mod_name]->set_lcm_utils(module_name,
                                                                        lcm_drv, fdt);
            }
        }
    }
...
{
    unsigned int ev = (unsigned int)event;
    ASSERT(dp_handle != NULL);
    struct ddp_path_handle_t *handle = (struct ddp_path_handle_t *)dp_handle;
    struct DPMGR_WQ_HANDLE *wq_handle = &handle->wq_list[ev];

    DISP_LOG_I("enable event on scenario %s, event %d, irtbit 0x%x\n",
               ddp_get_scenario_name(handle->scenario),
               event,
               handle->irq_event_map[ev].irq_bit);
    if (!wq_handle->init) {
...
int dpmgr_check_status(void *dp_handle)
{
    ...
    for (k = 0; k <= has_dual_path; k++) {
        if (k == 0) {
            ...
            ddp_dump_analysis(DISP1_MODULE_CONFIG);
            ddp_dump_analysis(DISP1_MODULE_MUTEX);
        }
        modules = ddp_get_scenario_list(scn[k]);
        module_num = ddp_get_module_num(scn[k]);
        for (i = 0; i < module_num; i++) {
        ...
        if (k == 0) {
            ...
            ddp_dump_reg(DISP1_MODULE_CONFIG);
            ddp_dump_reg(DISP1_MODULE_MUTEX);
        }
    }
    return 0;
}
...
int dpmgr_wait_event_timeout(void *dp_handle, enum DISP_PATH_EVENT event, int timeout)
{
    ...
    enum DISP_MODULE_ENUM module = IRQBIT_MODULE(irqmap[ev].irq_bit);
    int q_bit = IRQBIT_BIT(irqmap[ev].irq_bit);
    unsigned int mod = (unsigned int)module;
    ...
    if (wq_handle->init) {
        ...
        if (ddp_modules_driver[mod] != 0 &&
            ...
            ret = ddp_modules_driver[mod]->polling_irq(module, q_bit, timeout);
