...
static int md1_revert_sequencer_setting(void)
{
    ...
    /* wait sequencer done */
        ...
        spin(1000);
        if (count == 1000) {
            ALWAYS_LOG("[POWER OFF] wait sequencer fail,0x1C803200=0x%x,0x1C803204=0x%x,0x1C803208=0x%x,0x1C803310=0x%x\n",
...
static int md1_enable_sequencer_setting(void)
{
    ...
    while (1) {
        ...
        if (count == 1000) {
            ALWAYS_LOG("[POWER OFF] wait sequencer fail,0x1C803200=0x%x,0x1C803204=0x%x,0x1C803208=0x%x,0x1C803310=0x%x\n",
                ccci_read32(AOC_BASE + 0x200),
                ccci_read32(AOC_BASE + 0x204),
                ccci_read32(AOC_BASE + 0x208),
                ccci_read32(AOC_BASE + 0x310));
            return -2;
        }
    }
