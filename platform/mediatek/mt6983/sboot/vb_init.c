...
#if (SECURITY_LEVEL == 1)
static uint8_t g_3072_oemkey[OEM_3072_PUBK_SZ] = {OEM_3072_PUBK};
#else
/* SECURITY_LEVEL == 0 or SECURITY_LEVEL undefined case */
...
#if (SECURITY_LEVEL == 1)
    ret = sec_set_oemkey(g_3072_oemkey, OEM_3072_PUBK_SZ);
#else
    /* SECURITY_LEVEL == 0 or SECURITY_LEVEL undefined case */
    ret = sec_set_oemkey(g_oemkey, OEM_PUBK_SZ);
#endif
    if (ret != 0)
        dprintf(CRITICAL, "[SBC] Fail to set oemkey. (0x%x)\n", ret);


    plinfo_get_brom_header_block_size(&pl_start_addr);
    ret = sec_func_init(pl_start_addr);
    if (ret != 0)
        dprintf(CRITICAL, "[SBC] Fail to initialize sec function. (0x%x)\n", ret);
#endif
