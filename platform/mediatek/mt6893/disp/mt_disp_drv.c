...
static void mt_disp_init(void *lcd_pa_base, void *lcd_va_base, void *fdt)
{
    ...
    if (ret < 0 || ret >= MTK_LCM_NAME_LENGTH)
        dprintf(0, "%s %d snprintf failed, %d\n",
...
void disp_init(void *fdt)
{
    ...
    g_fb_base = mblock_alloc(ROUNDUP(g_fb_size, PAGE_SIZE), PAGE_SIZE,
                            0xFFFFFFFF, 0x80000000, 1, "framebuffer");
    if (!g_fb_base) {
    ...
    dprintf(0, "%s, %d, fb_base:0x%lx, fb_va:0x%lx, size:%u\n",
        __func__, __LINE__, (unsigned long)g_fb_base,
        (unsigned long)fb_base_va, g_fb_size);
