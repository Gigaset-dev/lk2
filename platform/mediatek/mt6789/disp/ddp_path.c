...
static struct mout_t mout_map[] = {
    ...
    {DISP_MODULE_DITHER0,
        {{DISP_MODULE_DSI0, 1 << 0},
         {DISP_MODULE_WDMA0, 1 << 1},
         {DISP_MODULE_DSC_WRAP0_CORE0, 1 << 2},
         {-1, 0} },
        0, 0},
...
static struct sel_t sel_in_map[] = {
    ...
    {DISP_MODULE_DSI0, {DISP_MODULE_RDMA0,
     DISP_MODULE_DITHER0,
     DISP_MODULE_DSC_WRAP0_CORE0, -1}, 0, 0},

    /* DISP_WDMA0_SEL_IN */
    {DISP_MODULE_WDMA0, {DISP_MODULE_DITHER0,
     DISP_MODULE_RSZ0, DISP_MODULE_OVL0_2L, DISP_MODULE_OVL0,
     DISP_MODULE_DSC_WRAP0_CORE0, -1}, 0, 0},
};
...
int ddp_path_init(void)
{
    ...
    mout_map[4].reg =
        (unsigned long *)DISP_DSC_WRAP0_MOUT_EN;
    ...
    sel_in_map[3].reg =
        (unsigned long *)DISP_WDMA0_SEL_IN;
...
static struct module_map_t module_mutex_map[DISP_MODULE_NUM] = {
    ...
    {DISP_MODULE_AAL0, 7, 0},
    {DISP_MODULE_GAMMA0, 8, 0},
    {DISP_MODULE_POSTMASK0, 9, 0},
    {DISP_MODULE_DITHER0, 10, 0},


    {DISP_MODULE_DSC_WRAP0_CORE0, 13, 0},
    {DISP_MODULE_DSI0, 14, 0},
    {DISP_MODULE_WDMA0, 15, 0},
            ...
            if (module_list[i] == sel_in_map[j].id) {
                ...
                for (k = 0; k < DDP_SOUT_MAX; k++) {
                    ...
                    if (sel_in_map[j].id_bit_map[k] == module_list[step]) {
                        DDPDBG("connect in_s %s to %s, value=%d, i:%d, j:%d\n",
                               ddp_get_module_name(module_list[step]),
                               ddp_get_module_name(module_list[i]), k, i, j);
