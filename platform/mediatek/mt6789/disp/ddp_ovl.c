...
static unsigned int ovl_offset(enum DISP_MODULE_ENUM module)
{
    unsigned int off = 0;
    ...
    case DISP_MODULE_OVL0:
        ...
        break;
    case DISP_MODULE_OVL0_2L:
        off = (unsigned int)DISP_OVL0_2L_BASE - (unsigned int)DISP_OVL0_BASE;
        break;
    default:
...
{
    unsigned int offset = ovl_offset(module);

#if 0
    //set red background
    DISP_REG_SET(handle, offset + DISP_REG_OVL_ROI_BGCLR, 0xFFFF0000);
#endif
    DISP_REG_SET_FIELD(handle, EN_FLD_OVL_EN, offset + DISP_REG_OVL_EN, 0x1);

    DISP_REG_SET(handle, offset + DISP_REG_OVL_INTEN,
             0x1E | REG_FLD_VAL(INTEN_FLD_ABNORMAL_SOF, 1));

    DISP_REG_SET_FIELD(handle, FLD_RDMA_BURST_CON1_BURST16_EN,
               offset + DISP_REG_OVL_RDMA_BURST_CON1, 1);
    DISP_REG_SET_FIELD(handle, DATAPATH_CON_FLD_LAYER_SMI_ID_EN,
               offset + DISP_REG_OVL_DATAPATH_CON, 1);
     ...
     *          offset + DISP_REG_OVL_FBDC_CFG1, 1);
     */
    DISP_REG_SET_FIELD(handle, EN_FLD_REG_BYPASS_SHADOW,
               offset + DISP_REG_OVL_EN, 1);
#if 0
    DDPMSG("ovl start done idx = %d, addr = 0x%x\n", idx, offset + DISP_REG_OVL_EN);
    DDPMSG("ovl start done addr1 = 0x%x\n", offset);
    DDPMSG("ovl start done addr2 = 0x%x\n", DISP_REG_OVL_EN);
    DDPMSG("ovl start done addr3 = 0x%x\n", offset + DISP_REG_OVL_EN);
...
int OVLStop(enum DISP_MODULE_ENUM module, void *handle)
{
    ...
    DISP_REG_SET(handle, offset + DISP_REG_OVL_INTEN, 0x00);
    DISP_REG_SET(handle, offset + DISP_REG_OVL_EN, 0x00);
    DISP_REG_SET(handle, offset + DISP_REG_OVL_INTSTA, 0x00);
...
#define OVL_IDLE (0x3)
    ...
    DISP_CPU_REG_SET(offset + DISP_REG_OVL_RST, 0x1);              // soft reset
    DISP_CPU_REG_SET(offset + DISP_REG_OVL_RST, 0x0);
    while (!(DISP_REG_GET(offset + DISP_REG_OVL_FLOW_CTRL_DBG) & OVL_IDLE)) {
        delay_cnt++;
        spin(10);
        if (delay_cnt > 2000) {
            DDPERR("OVL%d Reset() timeout!\n", idx);
            break;
        }
    }
    return 0;
           ...
           void *handle)
{
    ...
    DISP_REG_SET(handle, offset + DISP_REG_OVL_ROI_SIZE, bgH<<16 | bgW);

    DISP_REG_SET(handle, offset + DISP_REG_OVL_ROI_BGCLR, bgColor);
                   ...
                   void *handle)
{
    unsigned int offset = ovl_offset(module);
    unsigned int cur = 0;
    unsigned int addr = offset + DISP_REG_OVL_SRC_CON;

    ASSERT(layer <= 3);

    switch (layer) {
    case 0:
        cur = DISP_REG_GET_FIELD(SRC_CON_FLD_L0_EN, addr);
        if (cur != en)
            DISP_REG_SET_FIELD(handle, SRC_CON_FLD_L0_EN, addr, en);
        break;
    case 1:
        cur = DISP_REG_GET_FIELD(SRC_CON_FLD_L1_EN, addr);
        if (cur != en)
            DISP_REG_SET_FIELD(handle, SRC_CON_FLD_L1_EN, addr, en);
        break;
    case 2:
        cur = DISP_REG_GET_FIELD(SRC_CON_FLD_L2_EN, addr);
        if (cur != en)
            DISP_REG_SET_FIELD(handle, SRC_CON_FLD_L2_EN, addr, en);
        break;
    case 3:
        cur = DISP_REG_GET_FIELD(SRC_CON_FLD_L3_EN, addr);
        if (cur != en)
    ...
    if (cur != en)
        DDPMSG("module:%d OVL layer:%d %s, en:%u cur:%u, addr:0x%x\n",
            module, layer, en ? "enable" : "disable", en, cur, addr);
    return 0;
}
                   ...
                   void *handle)
{
    ...
    unsigned int idx_offset  = ovl_offset(module);
...
{
    int i = 0;
    unsigned int idx = ovl_index(module);
    ...
    if (idx >= OVL_NUM) {
        DDPERR("[%s]ovl_idx value is err: %d\n", __func__, idx);
        return;
    }
        ...
        enum DISP_MODULE_ENUM next, int connect, void *handle)
{
    ...
    if (connect && is_module_ovl(prev))
        ...
        DISP_REG_SET_FIELD(handle, DATAPATH_CON_FLD_BGCLR_IN_SEL,
                   offset + DISP_REG_OVL_DATAPATH_CON, 0);
...
void ovl_dump_golden_setting(enum DISP_MODULE_ENUM module)
{
    ...
    DDPDUMP("[9:0]:%u [25:16]:%u\n",
        DISP_REG_GET_FIELD(FLD_OVL_RDMA_FIFO_THRD,
            offset + DISP_REG_OVL_RDMA0_FIFO_CTRL),
    ...
    DDPDUMP("[11:0]:%u [27:16]:%u [28]:%u [29]:%u [30]:%u\n",
        DISP_REG_GET_FIELD(FLD_OVL_RDMA_MEM_GMC2_ISSUE_REQ_THRES,
            offset + DISP_REG_OVL_RDMA0_MEM_GMC_S2),
    ...
    DDPDUMP("[28]:%u\n",
        DISP_REG_GET_FIELD(FLD_RDMA_BURST_CON1_BURST16_EN,
            offset + DISP_REG_OVL_RDMA_BURST_CON1));
    ...
    DDPDUMP("[3:0]%u [7:4]%u [11:8]%u [15:12]%u [23:16]%x [26:24]%u\n",
        DISP_REG_GET_FIELD(FLD_OVL_RDMA_GREQ_LAYER0_GREQ_NUM,
            offset + DISP_REG_OVL_RDMA_GREQ_NUM),
    ...
    DDPDUMP("[3:0]:%u [7:4]:%u [11:8]:%u [15:12]:%u [25:16]:%u [28]:%u\n",
        DISP_REG_GET_FIELD(FLD_OVL_RDMA_GREQ_LAYER0_GREQ_URG_NUM,
            offset + DISP_REG_OVL_RDMA_GREQ_URG_NUM),
    ...
    DDPDUMP("[1:0]%u [3:2]%u [5:4]%u [7:6]%u [9:8]%u\n",
        DISP_REG_GET_FIELD(FLD_OVL_RDMA_PREULTRA_BUF_SRC,
            offset + DISP_REG_OVL_RDMA_ULTRA_SRC),
    ...
    DDPDUMP("[11:0]:%x [23:12]:%x\n",
        DISP_REG_GET_FIELD(FLD_OVL_RDMA_BUF_LOW_ULTRA_TH,
            offset + DISP_REG_OVL_RDMAn_BUF_LOW(0)),
    ...
    DDPDUMP("[23:12]:%x [31]:%x\n",
        DISP_REG_GET_FIELD(FLD_OVL_RDMA_BUF_HIGH_PREULTRA_TH,
            offset + DISP_REG_OVL_RDMAn_BUF_HIGH(0)),
    ...
    DDPDUMP("[0]:%u, [3]:%u [24]:%u [25]:%u [26]:%u\n",
        DISP_REG_GET_FIELD(DATAPATH_CON_FLD_LAYER_SMI_ID_EN,
            offset + DISP_REG_OVL_DATAPATH_CON),
...
void OVLDump(enum DISP_MODULE_ENUM module)
{
    ...
    module_base = offset + DISP_OVL0_OVL_STA;
...
void ovl_cal_golden_setting(unsigned int *gs)
{
    ...
    /* OVL_RDMA_GREQ_NUM */
    gs[GS_OVL_RDMA_GREQ_NUM] = 0xF1FF5555;

    /* OVL_RDMA_GREQURG_NUM */
    gs[GS_OVL_RDMA_GREQ_URG_NUM] = 0x5555;
...
static int ovl_golden_setting(enum DISP_MODULE_ENUM module, void *cmdq)
{
    ...
    for (i = 0; i < layer_num; i++) {
        Lx_base = i * OVL_LAYER_OFFSET + offset;
    ...
    for (i = 0; i < layer_num; i++)
        DISP_REG_SET(cmdq, offset + DISP_REG_OVL_RDMA0_MEM_GMC_S2 +
            i * 4, regval);

    /* DISP_REG_OVL_RDMA_GREQ_NUM */
    regval = gs[GS_OVL_RDMA_GREQ_NUM];
    DISP_REG_SET(cmdq, offset + DISP_REG_OVL_RDMA_GREQ_NUM, regval);

    /* DISP_REG_OVL_RDMA_GREQ_URG_NUM */
    regval = gs[GS_OVL_RDMA_GREQ_URG_NUM];
    DISP_REG_SET(cmdq, offset + DISP_REG_OVL_RDMA_GREQ_URG_NUM, regval);

    /* DISP_REG_OVL_RDMA_ULTRA_SRC */
    regval = gs[GS_OVL_RDMA_ULTRA_SRC];
    DISP_REG_SET(cmdq, offset + DISP_REG_OVL_RDMA_ULTRA_SRC, regval);
    ...
    for (i = 0; i < layer_num; i++)
        DISP_REG_SET(cmdq, offset + DISP_REG_OVL_RDMAn_BUF_LOW(i),
                 regval);
    ...
    for (i = 0; i < layer_num; i++)
        DISP_REG_SET(cmdq, offset + DISP_REG_OVL_RDMAn_BUF_HIGH(i),
                 regval);

    /* OVL_EN */
    regval = gs[GS_OVL_BLOCK_EXT_ULTRA] +
        (gs[GS_OVL_BLOCK_EXT_PRE_ULTRA] << 1);
    DISP_REG_SET_FIELD(cmdq, REG_FLD_MSB_LSB(19, 18), offset +
...
//        ovl_layer_config(module, local_layer, has_sec_layer, ovl_cfg, handle);
    ...
    DISP_REG_SET(handle, offset + DISP_REG_OVL_SRC_CON, enabled_layers);
