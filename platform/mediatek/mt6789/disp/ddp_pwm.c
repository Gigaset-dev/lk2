...
#ifndef CLK_CFG_9
#define CLK_CFG_9 (IO_BASE+0xA0)
#endif

#ifndef CLK_CFG_9_SET
#define CLK_CFG_9_SET (IO_BASE+0xA4)
#endif

#ifndef CLK_CFG_9_CLR
#define CLK_CFG_9_CLR (IO_BASE+0xA8)
#endif
...
void disp_pwm_init(enum disp_pwm_id_t id)
{
    ...
    if (config_data != NULL) {
        if (config_data->clock_source >= 0 && config_data->clock_source <= 4) {
            unsigned int reg_val = PWM_REG_GET(CLK_CFG_9);

            PWM_REG_SET(CLK_CFG_9_CLR, 0x7 << 16);
            PWM_REG_SET(CLK_CFG_9_SET, (0x7 & (config_data->clock_source << 0)) << 16);
            PWM_DBG("%s : CLK_CFG_9 0x%x => 0x%x", __func__, reg_val, PWM_REG_GET(CLK_CFG_9));
        }
        /*
