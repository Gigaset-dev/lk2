...
#define cw_printk(flg, fmt, arg...)        \
	({                                    \
		if(flg >= CWFG_ENABLE_LOG){\
			printf("FG_CW2015 : %s : " fmt, __FUNCTION__ ,##arg);\
		}else{}                           \
...
int cw2015_write_byte(unsigned char addr, unsigned char value)
{
    ...
    ret_code = mtk_i2c_write(CW2015_I2C_ID, CW2015_SLAVE_ADDR, write_data, len, NULL);
    if(NO_ERROR != ret_code)
...
int cw2015_read_byte(unsigned char addr, unsigned char *dataBuffer) 
{
    int ret_code = NO_ERROR;
    signed short len;
    *dataBuffer = addr;
    ...
    ret_code = mtk_i2c_write_read(CW2015_I2C_ID, CW2015_SLAVE_ADDR,dataBuffer, 1, 1, NULL);
...
#else
	//get i2c_id from dts
	offset = fdt_node_offset_by_compatible(fdt,-1,"cellwise,cw2015");
	if (offset < 0) {
		dprintf(CRITICAL, "[cw2015] Failed to find cellwise,cw2015 in dtb\n");
	}else{
		//batinfo
		data = fdt_getprop(fdt, offset, "batinfo", &len);
		if (len > 0 && data){
			dprintf(CRITICAL, "[cw2015] batinfo len(%d)\n",len);
			if (len != SIZE_BATINFO){
				dprintf(CRITICAL, "[cw2015] get bat batinfo fail len(%d) != SIZE_BATINFO(%d)\n",len,SIZE_BATINFO);
		...
		data = fdt_getprop(fdt, offset, "id", &len);
		if (len > 0 && data){
			cw2015_i2c_idinfo = fdt32_to_cpu(*(unsigned int *)data);
			dprintf(INFO, "[cw2015] i2c_id(%d)\n",cw2015_i2c_idinfo);
		}
