...
#define MT6373_PMIC_AUXADC_ADC_OUT_CH12_L_ADDR  (0x10d2)
#define MT6373_PMIC_SDMADC_EXT_THR_SRC_SEL_ADDR (0x11c4)
...
{ \
    ...
    .hw_info = { \
        ...
        .ratio = { 1, 1 }, \
        .enable_reg = MT6373_AUXADC_RQST1, \
        .enable_mask = (1 << 4), \
        .ready_reg = MT6373_PMIC_AUXADC_ADC_OUT_CH12_L_ADDR, \
        .ready_mask = AUXADC_RDY_BIT, \
        .value_reg = MT6373_PMIC_AUXADC_ADC_OUT_CH12_L_ADDR, \
        .res = 15, \
    }, \
    .sdmadc_hw_info = { \
        .set_reg =  MT6373_PMIC_SDMADC_EXT_THR_SRC_SEL_ADDR, \
        .cmd = EXT_THR_SEL_DEF_VALUE | src_sel, \
    }, \
}

static const struct auxadc_chan_spec mt6373_sdmadc_chan_specs[] = {
    SDMADC_CHAN_SPEC(VIN1, 1),
    SDMADC_CHAN_SPEC(VIN2, 2),
    SDMADC_CHAN_SPEC(VIN3, 3),
    SDMADC_CHAN_SPEC(VIN4, 4),
    SDMADC_CHAN_SPEC(VIN5, 5),
};
...
static int mt6373_sdmadc_read(const struct auxadc_device *auxadc_dev,
...
static struct auxadc_device mt6373_sdmadc_device = {
    .name = "second_pmic_sdmadc",
    .chans = mt6373_sdmadc_chan_specs,
    .num_chans = countof(mt6373_sdmadc_chan_specs),
    .write_regs = mt6373_auxadc_write_regs,
    .read_regs = mt6373_auxadc_read_regs,
    .sdmadc_read = mt6373_sdmadc_read,
};

void mt6373_sdmadc_init(void)
{
    int ret;
    ...
    ret = pmic_auxadc_device_register(&mt6373_sdmadc_device);
    if (ret < 0)
        dprintf(CRITICAL, "%s failed to register auxadc device\n", __func__);
}
LK_INIT_HOOK(mt6373_sdmadc, mt6373_sdmadc_init, LK_INIT_LEVEL_PLATFORM_EARLY + 1);
