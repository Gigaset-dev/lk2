...
#define PMIC_AUXADC_ADC_OUT_CH12_L_ADDR    (0x10d2)
#define PMIC_SDMADC_EXT_THR_SRC_SEL_ADDR   (0x11c4)
...
{ \
    ...
    .hw_info = { \
        ...
        .value_reg = PMIC_AUXADC_ADC_OUT_CH12_L_ADDR, \
        .res = 15, \
    }, \
    .sdmadc_hw_info = { \
...
static const struct auxadc_chan_spec mt6363_sdmadc_chan_specs[] = {
    ...
    SDMADC_CHAN_SPEC(VIN6, 6),
    SDMADC_CHAN_SPEC(VIN7, 7),
};
...
static int mt6363_sdmadc_read(const struct auxadc_device *auxadc_dev,
...
static struct auxadc_device mt6363_sdmadc_device = {
    .name = "main_pmic_sdmadc",
    .chans = mt6363_sdmadc_chan_specs,
    .num_chans = countof(mt6363_sdmadc_chan_specs),
    .write_regs = mt6363_auxadc_write_regs,
    .read_regs = mt6363_auxadc_read_regs,
    .sdmadc_read = mt6363_sdmadc_read,
};

static void mt6363_sdmadc_init(uint level)
{
    int ret;
    ...
    ret = pmic_auxadc_device_register(&mt6363_sdmadc_device);
    if (ret < 0)
        dprintf(CRITICAL, "%s failed to register auxadc device\n", __func__);
}
LK_INIT_HOOK(mt6363_sdmadc, mt6363_sdmadc_init, LK_INIT_LEVEL_PLATFORM_EARLY + 1);
