...
#define DEFAULT_RFG        100
#define UNIT_FG_CURRENT    (381470)
...
void fgauge_read_IM_current(void *data)
{
    ...
    Temp_Value = Temp_Value * UNIT_FG_CURRENT;
    ...
    /* Auto adjust value */
    if (fg_cust_data->r_fg_value != DEFAULT_RFG) {
        dvalue = (dvalue * DEFAULT_RFG) / fg_cust_data->r_fg_value;
        curr_rfg = dvalue;
    }
...
int fgauge_get_current(bool *fg_is_charging, int *data)
{
    ...
    LTRACEF("[fgauge_read_current],cartune:%d,%d,%d\n",
        fg_cust_data->car_tune_value, fg_cust_data->r_fg_value, UNIT_FG_CURRENT);
    ...
    while (!fg_get_data_ready_status()) {
        m++;
        if (m > 1000) {
    ...
    *data = dvalue;
    LTRACEF("[%s]is_charge:%d Ori_curr:0x%x,curr:[%d,%d,Final:%d] Rfg:%d ratio:%d\n",
        __func__, *fg_is_charging, uvalue16, ori_curr, curr_rfg, dvalue,
