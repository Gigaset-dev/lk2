...
static unsigned int get_sec_state(uint32_t *sboot_state, uint32_t *lock_state)
{
    ...
    if (ret != 0) {
        dprintf(CRITICAL, "[SEC] get sboot state fail, ret: %d\n", ret);
        goto end;
    }

    ret = sec_get_lock_state_adapter(lock_state);
    if (ret != 0) {
        dprintf(CRITICAL, "[SEC] get lock state fail, ret: %d\n", ret);
        goto end;
    }
...
static unsigned int get_sec_policy(unsigned int policy_entry_idx)
{
    ...
    if (ret) {
        dprintf(CRITICAL, "[SEC] Fail to get security state, just follow security policy.\n");
