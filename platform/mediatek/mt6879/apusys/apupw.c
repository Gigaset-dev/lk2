...
enum apupll4h_id {
    PLL1 = 0, // PLL1/FHCTL0/APUPLL: MDLA use
    PLL2 = 1, // PLL2/FHCTL1/NPUPLL: MVPU use
    PLL3 = 2, // PLL3/FHCTL2/APUPLL1: MNOC, uP use
    PLL4 = 3, // PLL4/FHCTL3/APUPLL2: Parking src use
    PLL4H_NUM
};

static void __apu_pll_init(void)
{
    int32_t pll_freq_out[PLL4H_NUM] = {832, 1000, 640, 210}; // MHz
    ...
    apu_writel(0xF, apupw.regs[apu_pll] + PLL4HPLL_FHCTL_HP_EN);
    // Hopping function reset release: ofs 0xE0C
    apu_writel(0xF, apupw.regs[apu_pll] + PLL4HPLL_FHCTL_RST_CON);
    // Hopping function clock enable: ofs 0xE08
    apu_writel(0xF, apupw.regs[apu_pll] + PLL4HPLL_FHCTL_CLK_CON);
    // Hopping function enable
    apu_setl((0x1 << 0) | (0x1 << 2),
            apupw.regs[apu_pll] + PLL4HPLL_FHCTL0_CFG); //ofs 0xE3C
    apu_setl((0x1 << 0) | (0x1 << 2),
            apupw.regs[apu_pll] + PLL4HPLL_FHCTL1_CFG); //ofs 0xE50
    apu_setl((0x1 << 0) | (0x1 << 2),
            apupw.regs[apu_pll] + PLL4HPLL_FHCTL2_CFG); //ofs 0xE64
    apu_setl((0x1 << 0) | (0x1 << 2),
            apupw.regs[apu_pll] + PLL4HPLL_FHCTL3_CFG); //ofs 0xE78

    // POSTDIV & PCW
    get_pll_pcw(pll_freq_out[PLL1], &posdiv_val, &pcw_val);
    apu_writel(((0x1 << 31) | (posdiv_val << 24) | pcw_val),
            apupw.regs[apu_pll] + PLL4H_PLL1_CON1); //POSTDIV: ofs 0x000C
    apu_writel(((0x1 << 31) | pcw_val),
            apupw.regs[apu_pll] + PLL4HPLL_FHCTL0_DDS); //PCW: ofs 0x0E44

    get_pll_pcw(pll_freq_out[PLL2], &posdiv_val, &pcw_val);
    apu_writel(((0x1 << 31) | (posdiv_val << 24) | pcw_val),
            apupw.regs[apu_pll] + PLL4H_PLL2_CON1); //POSTDIV: ofs 0x001C
    apu_writel(((0x1 << 31) | pcw_val),
            apupw.regs[apu_pll] + PLL4HPLL_FHCTL1_DDS); //PCW: ofs 0x0E58

    get_pll_pcw(pll_freq_out[PLL3], &posdiv_val, &pcw_val);
    apu_writel(((0x1 << 31) | (posdiv_val << 24) | pcw_val),
            apupw.regs[apu_pll] + PLL4H_PLL3_CON1); //POSTDIV: ofs 0x002C
    apu_writel(((0x1 << 31) | pcw_val),
            apupw.regs[apu_pll] + PLL4HPLL_FHCTL2_DDS); //PCW: ofs 0x0E6C

    get_pll_pcw(pll_freq_out[PLL4], &posdiv_val, &pcw_val);
    apu_writel(((0x1 << 31) | (posdiv_val << 24) | pcw_val),
            apupw.regs[apu_pll] + PLL4H_PLL4_CON1); //POSTDIV: ofs 0x003C
    apu_writel(((0x1 << 31) | pcw_val),
            apupw.regs[apu_pll] + PLL4HPLL_FHCTL3_DDS); //PCW: ofs 0x0E80
...
{
    struct spmi_device *mt6368_dev = NULL;
    uint8_t en_seq_off = (0x1 << BUCK_VAPU_PMIC_REG_EN_SHIFT);
    uint8_t en_seq_ret;

    mt6368_dev = get_spmi_device(SPMI_MASTER_1, SPMI_SLAVE_5);
    if (!mt6368_dev) {
