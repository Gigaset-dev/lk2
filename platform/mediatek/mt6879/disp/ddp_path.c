...
//#define DISPLAY_BRING_UP_STAGES
...
int primary_disp_dsc_module_list[DDP_ENING_NUM] = {
        ...
        DISP_MODULE_MAIN_OVL_PQ0_VIRTUAL, DISP_MODULE_RDMA0_POS_VIRTUAL,
        DISP_MODULE_RDMA0,
#ifndef DISPLAY_BRING_UP_STAGES
        DISP_MODULE_TDSHP0,
        DISP_MODULE_COLOR0, DISP_MODULE_CCORR0, DISP_MODULE_CCORR1,
        DISP_MODULE_C3D0,
        DISP_MODULE_AAL0, DISP_MODULE_GAMMA0, DISP_MODULE_POSTMASK0,
        DISP_MODULE_DITHER0, DISP_MODULE_CM0, DISP_MODULE_SPR0,
        DISP_MODULE_PQ0_VIRTUAL, DISP_MODULE_DSC_WRAP0, DISP_MODULE_MAIN0_VIRTUAL,
#endif
...
int module_list_scenario[DDP_SCENARIO_MAX][DDP_ENING_NUM] = {
    ...
    {
        DISP_MODULE_OVL0_2L, DISP_MODULE_OVL0,
        DISP_MODULE_MAIN_OVL_WDMA0_VIRTUAL, DISP_MODULE_WDMA0,
        DISP_MODULE_MAIN_OVL_PQ0_VIRTUAL, DISP_MODULE_RDMA0_POS_VIRTUAL,
        DISP_MODULE_RDMA0,
#ifndef DISPLAY_BRING_UP_STAGES
        DISP_MODULE_TDSHP0,
    ...
    {
        DISP_MODULE_OVL0_2L_NWCG, DISP_MODULE_DPI0,
        -1,
    },
    ...
    {
        DISP_MODULE_OVL0_2L_NWCG, DISP_MODULE_WDMA0,
        -1,
    },
...
static struct mout_t mout_map[] = {
    /* DISP_OVL0_2L_BLEND_MOUT_EN */
    {DISP_MODULE_OVL0_2L,
        {{DISP_MODULE_RSZ0, 1 << 0},
         {DISP_MODULE_MAIN_OVL_WDMA0_VIRTUAL, 1 << 2},
         {DISP_MODULE_MAIN_OVL_PQ0_VIRTUAL, 1 << 4},
         {-1, 0} },
        0, 0},
    ...
    {DISP_MODULE_POSTMASK0,
        {{DISP_MODULE_DITHER0, 1 << 0},
         {DISP_MODULE_CHIST0, 1 << 1},
         {DISP_MODULE_CHIST1, 1 << 2},
         {-1, 0} },
        0, 0},
...
static struct sel_t sel_in_map[] = {
    ...
    /* DISP_MAIN_OVL_DISP_WDMA_SEL_IN */
...
int ddp_path_init(void)
{
    ...
    /* mout */
    mout_map[0].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_OVL0_2L_BLEND_MOUT_EN;
    mout_map[1].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_OVL0_BLEND_MOUT_EN;
    mout_map[2].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_RSZ0_MOUT_EN;
    mout_map[3].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_RDMA0_POS_MOUT_EN;
    mout_map[4].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_POSTMASK0_MOUT_EN;
    mout_map[5].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_DITHER0_MOUT_EN;
    mout_map[6].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_SPR0_MOUT_EN;
    mout_map[7].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_MERGE0_MOUT_EN;
    mout_map[8].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_DSC_WRAP0_MOUT_EN;
    ...
    sel_in_map[1].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_MAIN_OVL_DISP_WDMA_SEL_IN;
    sel_in_map[2].reg =
    ...
    sel_in_map[9].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_CHIST0_SEL_IN;
    sel_in_map[10].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_CHIST1_SEL_IN;
    sel_in_map[11].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_MERGE0_L_SEL_IN;
    sel_in_map[12].reg =
    ...
    sel_in_map[16].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_UFBC_WDMA0_SEL_IN;
...
static struct module_map_t module_mutex_map[DISP_MODULE_NUM] = {
    ...
    //{DISP_MODULE_OVL1_2L, 2, 0},
    {DISP_MODULE_RSZ0, 3, 0},
    {DISP_MODULE_RDMA0, 4, 0},
    {DISP_MODULE_TDSHP0, 5, 0},
    {DISP_MODULE_C3D0, 6, 0},
    ...
    {DISP_MODULE_DSC_WRAP0, 19, 0},
    {DISP_MODULE_DSC_WRAP0_CORE1, 20, 0},
    {DISP_MODULE_MERGE0, 21, 0},
    {DISP_MODULE_DSI0, 22, 0},
    {DISP_MODULE_WDMA0, 23, 0},
    {DISP_MODULE_UFBC_WDMA0, 24, 0},
    {DISP_MODULE_OVL0_2L_NWCG, 25, 0},
    {DISP_MODULE_OVL1_2L_NWCG, 26, 0},
    {DISP_MODULE_RDMA1, 27, 0},
    {DISP_MODULE_DP_INTF0, 28, 0},
    {DISP_MODULE_DPI0, 29, 0},
    {DISP_MODULE_WDMA1, 30, 0},
    {DISP_MODULE_Y2R0, 31, 0},

    {DISP_MODULE_INLINEROT0, 0, 1},
    {DISP_MODULE_DLI_ASYNC0, 1, 1},
    {DISP_MODULE_DLI_ASYNC1, 2, 1},
    {DISP_MODULE_DLI_ASYNC2, 3, 1},
    {DISP_MODULE_DLI_ASYNC3, 4, 1},
    {DISP_MODULE_DLO_ASYNC0, 5, 1},
    {DISP_MODULE_DLO_ASYNC1, 6, 1},
    {DISP_MODULE_DLO_ASYNC2, 7, 1},
    {DISP_MODULE_DLO_ASYNC3, 8, 1},
    ...
    {DISP_MODULE_MAIN_OVL_WDMA0_VIRTUAL, -1, 0},
    {DISP_MODULE_MAIN_OVL_PQ0_VIRTUAL, -1, 0},
    {DISP_MODULE_RDMA0_POS_VIRTUAL, -1, 0},
    {DISP_MODULE_MAIN0_VIRTUAL, -1, 0},
...
static void ddp_connect_path_l(int *module_list, void *handle)
{
    ...
    for (int i = 0; i < module_num - 1; i++) {
            ...
            if (module_list[i] == mout_map[j].id) {
                ...
                for (k = 0; k < DDP_MOUT_MAX; k++) {
                    ...
                    if (mout_map[j].out_id_bit_map[k].m == module_list[step]) {
                        ...
                        reg_mout |= mout;
                        mout_idx = j;
                        DDPDBG("connect mout %s to %s  bits = 0x%x, i = %d, j = %d\n",
        ...
        if (reg_mout) {
            ...
            DDPDBG("connect mout reg=0x%lx val=0x%x\n", (unsigned long)mout_map[mout_idx].reg,
                    DISP_REG_GET(mout_map[mout_idx].reg));
            reg_mout = 0;
            mout_idx = 0;
        }
            ...
            if (module_list[i] == sel_out_map[j].id) {
                ...
                for (k = 0; k < DDP_SOUT_MAX; k++) {
                    ...
                    if (sel_out_map[j].id_bit_map[k] == module_list[step]) {
                        DDPDBG("connect out_s %s to %s, value = %d, i = %d, j = %d\n",
                               ddp_get_module_name(module_list[i]),
                               ddp_get_module_name(module_list[step]), k, i, j);
...
static void ddp_print_scenario(enum DDP_SCENARIO_ENUM scenario)
{
    ...
    for (i = 0; i < num; i++) {
        len = sprintf(module_name, "%d", module_list_scenario[scenario][i]);
        if (len >= 0 && (len + strlen(path) < sizeof(path)))
...
int ddp_set_dst_module(enum DDP_SCENARIO_ENUM scenario, enum DISP_MODULE_ENUM dst_module)
{
    ...
    if (dst_module == DISP_MODULE_DSIDUAL) {
        DDPERR("dst module is DISP_MODULE_DSIDUAL which is not supported\n");
        return -1;
    } else {
...
static void ddp_smi_config(void)
{
    ...
    DDPMSG("SMI subcom before config:(0x%lx,0x%lx)=(0x%x,0x%x)\n",
        SMI_SUBCOM0_BASE + offset,
        SMI_SUBCOM1_BASE + offset,
        DISP_REG_GET(SMI_SUBCOM0_BASE + offset),
        DISP_REG_GET(SMI_SUBCOM1_BASE + offset));
    DDPMSG("SMI subcom before config:(0x%lx,0x%lx)=(0x%x,0x%x)\n",
        SMI_SUBCOM2_BASE + offset,
        SMI_SUBCOM3_BASE + offset,
        DISP_REG_GET(SMI_SUBCOM2_BASE + offset),
        DISP_REG_GET(SMI_SUBCOM3_BASE + offset));


    DISP_REG_SET(NULL, SMI_SUBCOM0_BASE + offset, value);
    DISP_REG_SET(NULL, SMI_SUBCOM1_BASE + offset, value);
    DISP_REG_SET(NULL, SMI_SUBCOM2_BASE + offset, value);
    DISP_REG_SET(NULL, SMI_SUBCOM3_BASE + offset, value);

    DDPMSG("SMI subcom after config:(0x%lx,0x%lx)=(0x%x,0x%x)\n",
        SMI_SUBCOM0_BASE + offset,
        SMI_SUBCOM1_BASE + offset,
        DISP_REG_GET(SMI_SUBCOM0_BASE + offset),
        DISP_REG_GET(SMI_SUBCOM1_BASE + offset));
