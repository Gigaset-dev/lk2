    ...
    unsigned int is_vdo)
{
    ...
    //smaller rdma fifo size because dsi buf is enabled
    gs[GS_RDMA_FIFO_SIZE] = 0x20;

    //disable underflow en because dsi buf is enabled
    ...
    void *handle)
{
    ...
    DISP_REG_SET(handle, offset + DISP_REG_RDMA_MEM_GMC_SETTING_4, 0x01);
            DISP_REG_SET_FIELD(handle, FLD_RG_PREULTRA_RDMA_SRC,
            offset + DISP_REG_RDMA_ULTRA_SRC_SEL, 0x00);
    DISP_REG_SET_FIELD(handle, FLD_RG_ULTRA_RDMA_SRC,
            offset + DISP_REG_RDMA_ULTRA_SRC_SEL, 0x00);
...
void rdma_dump_golden_setting(enum DISP_MODULE_ENUM module)
{
    ...
    DDPDUMP("GREQ_URG_NUM_SEL [29]:%u\n",
        DISP_REG_GET_FIELD(FLD_RG_LAYER_SMI_ID_EN,
            offset + DISP_REG_RDMA_GREQ_URG_NUM_SEL));
                        ...
                        void *handle)
{
    ...
    LCM_DSC_CONFIG_PARAMS *dsc_params = &dsi->dsc_params;
    ...
    if (pConfig->dst_dirty) {
        ...
        if (dsc_params->bit_per_channel == 10)
            DISP_REG_SET_FIELD(handle, GLOBAL_CON_FLD_RG_PIXEL_10_BIT,
