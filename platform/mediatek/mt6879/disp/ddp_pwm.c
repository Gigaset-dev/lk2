...
#ifndef CLK_CFG_7
#define CLK_CFG_7 (IO_BASE+0x80)
#endif

#ifndef CLK_CFG_7_SET
#define CLK_CFG_7_SET (IO_BASE+0x84)
#endif

#ifndef CLK_CFG_7_CLR
#define CLK_CFG_7_CLR (IO_BASE+0x88)
#endif
...
void disp_pwm_init(enum disp_pwm_id_t id)
{
    ...
    if (config_data != NULL) {
        if (config_data->clock_source >= 0 && config_data->clock_source <= 4) {
            ...
            PWM_REG_SET(CLK_CFG_7_CLR, 0x7 << 24);
            PWM_REG_SET(CLK_CFG_7_SET, (0x7 & (config_data->clock_source << 0)) << 24);
            PWM_DBG("%s : CLK_CFG_7 0x%x => 0x%x", __func__, reg_val, PWM_REG_GET(CLK_CFG_7));
        }
        /*
