...
static int disp_lcm_check_compare_id(void *fdt, LCM_DRIVER *lcm_drv, char *lcm_name)
{
...
#ifdef MTK_RUNTIME_SWITCH_FPS_SUPPORT
    {
        ...
        if (fdt)
            target_fps = disp_lcm_get_dts_u32(fdt, "/dsi", "switch_fps");
        if (target_fps > 0) {
            if (lcm_params->dsi.fps != target_fps) {
                DISPMSG("%s: lcm:%s found with fps:%u didn't match target fps:%u\n",
                    __func__, lcm_drv->name, lcm_params->dsi.fps, target_fps);
                goto FAIL1;
            } else {
                DISPMSG("%s: lcm:%s found with fps:%u, target fps:%u\n",
                    __func__, lcm_drv->name, lcm_params->dsi.fps, target_fps);
            }
        }
    }
...
FAIL2:
    LCM_DBG("%s: we have finished checking compare id of \"%s\", ret=%d\n",
...
#ifdef MTK_RUNTIME_SWITCH_SPR_SUPPORT
static unsigned int disp_get_dts_spr_status(void *fdt)
{
    int nodeoffset, data, need_prefix;
    const void *val = NULL;
    char buf[100];
    int ret;
    ...
    if (ret < 0 || ret >= sizeof(buf) - 1) {
        DISPERR("%s,snprint dst_dsi failed\n", __func__);
        goto error;
    }
    ...
    if (need_prefix)
        nodeoffset = fdt_path_offset(fdt, "/soc/dsi");
    else
        nodeoffset = fdt_path_offset(fdt, "/dsi");
    ...
    val = fdt_getprop(fdt, nodeoffset, "switch_spr", NULL);
    if (val) {
        memcpy(&data, val, sizeof(data));
        data = fdt32_to_cpu(data);
        DISPMSG("default disp switch_spr = <%d>\n", data);
        return (unsigned int)data;
    } else {
        DISPMSG("default disp switch_spr in dtb is empty\n");
    }
...
struct disp_lcm_handle *disp_lcm_probe(char *plcm_name, void *fdt)
{
    ...
    if (strlen(&lcm_name[0]) > 0) {
        ...
        if (ret != 0) {
            ...
            LCM_MSG("%s, %d: we will use panel:\"%s\", is_checked:%d\n",
                __func__, __LINE__, lcm_name, is_checked);
            isLCMFound = true;
        }
    } else {
        LCM_MSG("%s, %d, failed to get boot up device\n",
            __func__, __LINE__);
    }
    ...
    while (mtk_lcm_drv_parse_next_lcm_dts(fdt, func, &lcm_name[0], force) >= 0) {
        ...
        if (disp_lcm_check_compare_id(fdt, lcm_drv, &lcm_name[0]) == 1) {
            LCM_MSG("%s, %d: we will use panel:\"%s\"\n",
                __func__, __LINE__, lcm_name);
            isLCMFound = true;
    ...
    if (isLCMFound == false)
        DISPERR("%s, %d, we have check all lcm list but failed\n",
            __func__, __LINE__);
            ...
            if (lcm_drv->compare_id != NULL) {
                if (lcm_drv->compare_id(fdt) != 0) {
                    ...
                    if (fdt)
                        target_fps = disp_lcm_get_dts_u32(fdt, "/dsi", "switch_fps");
                    if (target_fps < 0)
                        target_fps = disp_lcm_get_dts_u32(fdt, "/soc/dsi", "switch_fps");
                    if (target_fps > 0) {
...
#ifdef MTK_RUNTIME_SWITCH_SPR_SUPPORT
                     unsigned int spr_status = 2;

                    if (fdt)
                        spr_status = disp_get_dts_spr_status(fdt);
                    if (spr_status < 2) {
                        if (hlcm_param.dsi.spr_enable != spr_status) {
                            DISPMSG("lcm:%s found with spr:%u didn't match target spr:%u\n",
                                lcm_drv->name, hlcm_param.dsi.spr_enable, spr_status);
                        _display_interface_path_deinit(handle);
                        continue;
                        } else {
                            DISPMSG("lcm:%s found with spr:%u, target spr:%u\n",
                                lcm_drv->name, hlcm_param.dsi.spr_enable, spr_status);
                        }
                    }
#endif
