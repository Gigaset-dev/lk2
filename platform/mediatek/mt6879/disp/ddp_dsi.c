                                 ...
                                 int w, int h)
{
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        ...
        if (dsi_params->spr_enable && spr_params->relay == 0) {
            DISP_REG_SET_FIELD(cmdq, DSI_PSCON_FLD_RG_XY_SWAP,
                            DSI_REG_BASE[i] + DISP_REG_DSI_PSCON,
...
#if 0
    //dsi pattern
    DSI_BIST_Pattern_Test(module, NULL, 1, 0x0fff0001);
    DSI_DumpRegisters(module, 1);
...
long lcd_enp_bias_setting(unsigned int value, void *fdt)
{
    ...
    if (ret != 0) {
        LCM_MSG("%s, gate ic power %s failed, %ld\n",
