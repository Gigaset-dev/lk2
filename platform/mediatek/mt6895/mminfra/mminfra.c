...
//#define ENABLE_GCE_VA
//#define ENABLE_MMUP_VA
#define ENABLE_GCE_MMUP_VA
//#define MMINFRA_DBG
...
void mminfra_init(void)
{
    ...
    dprintf(CRITICAL, "%s mdp0=0x%x mdp1=0x%x\n", __func__, mm_read(MMINFRA_MDP_AID_REMAP),
        mm_read(MMINFRA_MDP_AID_REMAP+4));
    dprintf(CRITICAL, "%s gce0=0x%x gce1=0x%x gce2=0x%x gce3=0x%x gce4=0x%x\n", __func__,
    ...
    dprintf(CRITICAL, "%s disp0=0x%x disp1=0x%x\n", __func__,
    ...
    dprintf(CRITICAL, "%s vmid0=0x%x vmid1=0x%x\n", __func__,
        mm_read(MMINFRA_VMID_REMAP), mm_read(MMINFRA_VMID_REMAP+0x2C));

    // Enable dom2aid
    mm_write(MMINFRA_AID_REMAP_DOM2AID_EN, mm_read(MMINFRA_AID_REMAP_DOM2AID_EN) | 0x1);

    // Rfifo threshold setting
...
#if defined(ENABLE_GCE_VA)
    mm_write(MMINFRA_AID_REMAP_DOM2AID_EN, mm_read(MMINFRA_AID_REMAP_DOM2AID_EN) | (0xF << 3));
#endif
#if defined(ENABLE_MMUP_VA)
    mm_write(MMINFRA_AID_REMAP_DOM2AID_EN, mm_read(MMINFRA_AID_REMAP_DOM2AID_EN) | (0x3 << 7));
#endif
    dprintf(CRITICAL, "%s MMINFRA_AID_REMAP_DOM2AID_EN=0x%x\n",
        __func__, mm_read(MMINFRA_AID_REMAP_DOM2AID_EN));
