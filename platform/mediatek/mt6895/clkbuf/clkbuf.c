...
int clk_buf_init(const void *fdt)
{
    ...
    int len;
    int phandle;
    enum CLKBUF_TCXO_STA tcxo_sta = TCXO_CLKBUF_NOT_SUPPORT;

    ASSERT(fdt != NULL);

    nodeoffset = fdt_node_offset_by_compatible(fdt, -1, CLKBUF_NODE);
    if (nodeoffset < 0) {
        tcxo_sta = -TCXO_CLKBUF_PROP_NOT_FOUND;
    ...
    if (tcxo_support && !strcmp(tcxo_support, "true")) {
        clk_buf_tcxo_config();
        tcxo_sta = TCXO_CLKBUF_SUPPORT;
        goto TCXO_INIT_DONE;
    }
TCXO_NOT_SUPPORT:
    dprintf(CRITICAL, "[CLKBUF] TCXO not support, id: %d\n", tcxo_sta);
