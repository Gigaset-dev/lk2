...
static unsigned int rdma_dispsys_index(enum DISP_MODULE_ENUM module)
{
    unsigned int idx = 0;
...
int RDMAStart(enum DISP_MODULE_ENUM module, void *handle)
{
    ...
    unsigned int disp_idx = rdma_dispsys_index(module);
    unsigned int regval;
    ...
    DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
            DISP_REG_RDMA_INT_ENABLE, regval);
    DISP_REG_SET_FIELD(handle, GLOBAL_CON_FLD_ENGINE_EN,
        disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
...
int RDMAReset(enum DISP_MODULE_ENUM module, void *handle)
{
    ...
    DISP_REG_SET_FIELD(handle, GLOBAL_CON_FLD_SOFT_RESET,
            disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
            DISP_REG_RDMA_GLOBAL_CON, 1);
    while ((DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_GLOBAL_CON)
                & 0x700) == 0x100) { //polling RESET_STATE is 0
        delay_cnt++;
        if (delay_cnt > 10000) {
            DDPERR("%s(%d) timeout, stage 1! DISP_REG_RDMA_GLOBAL_CON=0x%x\n", __func__, idx,
                    DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                        DISP_REG_RDMA_GLOBAL_CON));
            break;
        }
    }
    ...
    delay_cnt = 0;
    while ((DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_GLOBAL_CON) & 0x700)
            != 0x100) {
        delay_cnt++;
        if (delay_cnt > 10000) {
            DDPERR("%s(%d) timeout, stage 2! DISP_REG_RDMA_GLOBAL_CON=0x%x\n", __func__,
                    idx, DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET +
...
void rdma_set_ultra(unsigned int disp_idx, unsigned int idx, unsigned int width,
                unsigned int height, unsigned int bpp, unsigned int frame_rate,
                unsigned int dsi_mode, void *handle)
{
    unsigned int gs[GS_RDMA_FLD_NUM] = {0};
    unsigned int val = 0;
    unsigned int offset = disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET;
    ...
    /* mmsys0 rdma consider the ultra signal from dsi buffer
     * mmsys1 rdma ignore the ultra signal from dsi buffer, since hw not connected
     */
    if (disp_idx == 0)
        DISP_REG_SET(handle, offset + DISP_REG_RDMA_MEM_GMC_SETTING_4, 0x1);
               ...
               void *handle)  // ourput setting
{
    ...
    if (input_is_yuv == 1 && output_is_yuv == 0) {
            ...
            DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                            DISP_REG_RDMA_PRE_ADD_0,
                            coef_rdma_601_y2r[3][0]);
            DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                            DISP_REG_RDMA_PRE_ADD_1,
                            coef_rdma_601_y2r[3][1]);
            DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                            DISP_REG_RDMA_PRE_ADD_2,
                            coef_rdma_601_y2r[3][2]);
            DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                            DISP_REG_RDMA_POST_ADD_0,
                            coef_rdma_601_y2r[4][0]);
            DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                            DISP_REG_RDMA_POST_ADD_1,
                            coef_rdma_601_y2r[4][1]);
            DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                            DISP_REG_RDMA_POST_ADD_2,
    ...
    DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_MEM_START_ADDR, address);
    DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_MEM_SRC_PITCH, pitch);
    DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_INT_ENABLE, 0x1F);
    ...
    rdma_set_ultra(disp_idx, idx, width, height, bpp, 60, dsi_mode, handle);
#if 0
    ...
    DISP_REG_SET_FIELD(handle, FIFO_CON_FLD_FIFO_PSEUDO_SIZE, disp_idx * DISPSYS_RDMA_OFFSET +
                    idx * DISP_RDMA_INDEX_OFFSET
...
int RDMAWait(enum DISP_MODULE_ENUM module, void *handle)
{
    ...
    while ((DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                                    DISP_REG_RDMA_INT_STATUS) & 0x4) != 0x4) {
            cnt++;
            if (cnt > 1000) {
                    DDPERR("RDMA%dWait timeout!\n", idx);
                    DISP_REG_SET(handle, disp_idx * DISPSYS_RDMA_OFFSET +
                                    idx * DISP_RDMA_INDEX_OFFSET +
                                    DISP_REG_RDMA_INT_STATUS, 0x0);
                    return -1;
            }
    }
...
static int RDMAPollingInterrupt(enum DISP_MODULE_ENUM module, int bit, int timeout)
{
    ...
    if (timeout <= 0) {
            while ((DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                                            DISP_REG_RDMA_INT_STATUS) & bit) == 0)
            ;
        cnt = 1;
    } else {
        // time need to update
        ...
        while (cnt > 0) {
            cnt--;
            regval = DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                            DISP_REG_RDMA_INT_STATUS);
            if (regval & bit) {
                    DISP_CPU_REG_SET(disp_idx * DISPSYS_RDMA_OFFSET +
                                    idx * DISP_RDMA_INDEX_OFFSET +
                                    DISP_REG_RDMA_INT_STATUS, ~regval);
                    break;
            }
            spin(100);
        }
...
void rdma_dump_golden_setting(enum DISP_MODULE_ENUM module)
{
    ...
    DDPDUMP("GMC_SETTING_3 [11:0]:%u [27:16]:%u\n",
        ...
        DISP_REG_GET_FIELD(FLD_HIGH_FOR_URGENT,
            offset + DISP_REG_RDMA_MEM_GMC_SETTING_3));
    DDPDUMP("GMC_SETTING_4 [0]:%u\n",
        DISP_REG_GET_FIELD(FLD_RDMA_DSI_BUFF_ULTRA_SEL,
            offset + DISP_REG_RDMA_MEM_GMC_SETTING_4));
}
...
void rdma_dump_analysis(enum DISP_MODULE_ENUM module)
{
    unsigned int idx = rdma_index(module);
    unsigned int disp_idx = rdma_dispsys_index(module);
    unsigned int global_ctrl = DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET +
                    idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_GLOBAL_CON);
    unsigned int bg0 = DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_BG_CON_0);
    unsigned int bg1 = DISP_REG_GET(disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
    ...
    DDPDUMP("rdma%d: en=%d,memory_mode=%d,smi_busy=%d,w=%d,h=%d,pitch=%d,addr=0x%x,fmt=%s\n",
                    ...
                    DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 +
                            disp_idx * DISPSYS_RDMA_OFFSET +
                            idx * DISP_RDMA_INDEX_OFFSET) & 0xfff,
                    DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_1 +
                            disp_idx * DISPSYS_RDMA_OFFSET +
                            idx * DISP_RDMA_INDEX_OFFSET) & 0xfffff,
                    ...
                    rdma_intput_format_name((DISP_REG_GET(DISP_REG_RDMA_MEM_CON +
                                            disp_idx * DISPSYS_RDMA_OFFSET +
                                            idx * DISP_RDMA_INDEX_OFFSET) >> 4) & 0xf));
                        ...
                        void *handle)
{
    ...
    unsigned int idx = rdma_index(module);
    unsigned int disp_idx = rdma_dispsys_index(module);
    unsigned long long offset = disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET;
    ...
    if (pConfig->dst_dirty) {
        ...
        if (mboot_params_current_abnormal_boot() &&
            aee_check_enable() == AEE_ENABLE_FULL) {
            DISP_REG_SET(handle, offset + DISP_REG_RDMA_BG_CON_1,
                         ((pConfig->dst_h - (pConfig->dst_h / 8)) << 16));
            DISP_REG_SET(handle, offset + DISP_REG_RDMA_BG_CON_2, 0xFFFF7F00);
            DISP_REG_SET_FIELD(handle, SIZE_CON_1_FLD_OUTPUT_FRAME_HEIGHT,
               offset + DISP_REG_RDMA_SIZE_CON_1, (pConfig->dst_h / 8));
        }
...
void rdma_enable_color_transform(enum DISP_MODULE_ENUM module)
{
    ...
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET +
                    idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_SIZE_CON_0, value);
}
...
void rdma_disable_color_transform(enum DISP_MODULE_ENUM module)
{
    ...
    u32 value =  DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 + disp_idx * DISPSYS_RDMA_OFFSET +
                    idx * DISP_RDMA_INDEX_OFFSET);

    value = value | REG_FLD_VAL((SIZE_CON_0_FLD_MATRIX_EXT_MTX_EN), 0) |
            REG_FLD_VAL((SIZE_CON_0_FLD_MATRIX_ENABLE), 0);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_SIZE_CON_0, value);
}
                           ...
                           struct rdma_color_post *post)
{
    ...
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_C00, matrix->C00);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_C01, matrix->C01);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_C02, matrix->C02);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_C10, matrix->C10);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_C11, matrix->C11);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_C12, matrix->C12);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_C20, matrix->C20);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_C21, matrix->C21);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_C22, matrix->C22);

    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_PRE_ADD_0, pre->ADD0);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_PRE_ADD_1, pre->ADD1);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_PRE_ADD_2, pre->ADD2);

    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_POST_ADD_0, post->ADD0);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_POST_ADD_1, post->ADD1);
    DISP_REG_SET(NULL, disp_idx * DISPSYS_RDMA_OFFSET + idx * DISP_RDMA_INDEX_OFFSET +
                    DISP_REG_RDMA_POST_ADD_2, post->ADD2);
}
