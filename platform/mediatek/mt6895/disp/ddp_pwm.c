...
#ifndef CLK_CFG_10
#define CLK_CFG_10 (IO_BASE+0xB0)
#endif

#ifndef CLK_CFG_10_SET
#define CLK_CFG_10_SET (IO_BASE+0xB4)
#endif

#ifndef CLK_CFG_10_CLR
#define CLK_CFG_10_CLR (IO_BASE+0xB8)
#endif
...
void disp_pwm_init(enum disp_pwm_id_t id)
{
    ...
    if (config_data != NULL) {
        if (config_data->clock_source >= 0 && config_data->clock_source <= 4) {
            unsigned int reg_val = PWM_REG_GET(CLK_CFG_10);

            PWM_REG_SET(CLK_CFG_10_CLR, 0x7 << 0);
            PWM_REG_SET(CLK_CFG_10_SET, (0x7 & (config_data->clock_source << 0)) << 0);
            PWM_DBG("%s : CLK_CFG_10 0x%x => 0x%x", __func__, reg_val, PWM_REG_GET(CLK_CFG_10));
        }
        /*
