...
static struct DSI_CMDQ_REGS *const DSI_CMDQ_REG[2] = {
                                           (struct DSI_CMDQ_REGS *)(DISPSYS_DSI0_BASE+0xd00),
                                           (struct DSI_CMDQ_REGS *)(DISPSYS_DSI1_BASE+0xd00)};

static struct DSI_VM_CMDQ_REGS *const DSI_VM_CMD_REG[2] = {
                                           (struct DSI_VM_CMDQ_REGS *)(DISPSYS_DSI0_BASE + 0x208),
                                           (struct DSI_VM_CMDQ_REGS *)(DISPSYS_DSI1_BASE + 0x208)};
...
enum DSI_STATUS DSI_SetMode(enum DISP_MODULE_ENUM module, void *cmdq, unsigned int mode)
{
    unsigned int i = 0;
...
void DSI0_WaitNotBusyAfterTrigger(enum DISP_MODULE_ENUM module, int force)
{
    unsigned int i = 0, cnt = 0;
    int delay_in_us = 50;
    unsigned int tmp = 0;
    ...
    /*...dsi video is always in busy state,so dsi_start set 0,then wait...*/
    if (_dsi_is_video_mode(module) && !force)
        return;

    delay_time = (_dsi_is_video_mode(module)) ?
                        (33 * 1000) : (1000 * 1000);
    i = DSI_MODULE_BEGIN(module);
    while (cnt < (delay_time / delay_in_us)) { /* poll 33ms */
        tmp = readl(DSI_REG_BASE[i] + DISP_REG_DSI_INTSTA);
        spin(delay_in_us);
        if (!(tmp & 0x80000000)) {
            DDPMSG("%s- cnt:%d\n", __func__, cnt);
...
void DSI0_Wait_TE(enum DISP_MODULE_ENUM module, int force)
{
    unsigned int i = 0, cnt = 0;
    int delay_in_us = 2;
    unsigned int tmp = 0;
    ...
    i = DSI_MODULE_BEGIN(module);
    DISP_REG_SET_FIELD(NULL, DSI_INTSTA_FLD_TE_RDY_INT_FLAG,
                        DSI_REG_BASE[i] + DISP_REG_DSI_INTSTA, 0);

    delay_time = (_dsi_is_video_mode(module)) ?
                        (33 * 1000) : (1000 * 1000);
    while (cnt < (delay_time / delay_in_us)) { /* poll 33ms */
        tmp = DISP_REG_GET_FIELD(DSI_INTSTA_FLD_TE_RDY_INT_FLAG,
                                 DSI_REG_BASE[i] + DISP_REG_DSI_INTSTA);
        if (tmp) {
            DDPMSG("%s- cnt:%d\n", __func__, cnt);
            return;
        }
...
bool DSI_clk_HS_state(enum DISP_MODULE_ENUM module, void *cmdq)
{
    ...
    memset(&tmpreg, 0, sizeof(struct DSI_PHY_LCCON_REG));
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
...
void DSI_clk_HS_mode(enum DISP_MODULE_ENUM module, void *cmdq, bool enter)
{
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        if (enter) {
            if (_dsi_context[i].dsi_params.IsCphy)
                DISP_REG_SET(NULL, DISP_REG_DSI_PHY_LCPAT + DSI_REG_BASE[i], 0xAA);
            else
    ...
    if (level >= 1) {
        for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
            ...
            for (k = 0; k < 32; k += 16) { /* only dump first 32 bytes cmd */
                DDPDUMP("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", 0xd00 + k,
                    readl((dsi_base_addr + 0xd00 + k)),
                    readl((dsi_base_addr + 0xd00 + k + 0x4)),
                    readl((dsi_base_addr + 0xd00 + k + 0x8)),
                    readl((dsi_base_addr + 0xd00 + k + 0xc)));
            }

#ifndef PROJECT_TYPE_FPGA
            DDPDUMP("== DSI_PHY%d REGS ==\n", i);
            for (k = 0; k < 0x6F0; k += 16) {
                DDPDUMP("0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", k,
...
enum DSI_STATUS DSI_BIST_Pattern_Test(enum DISP_MODULE_ENUM module, void *cmdq,
        bool enable, unsigned int value)
{
    unsigned int i = 0;
    void *temp;
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        if (enable) {
            DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_BIST_PATTERN, value);
            ...
            if (!_dsi_is_video_mode(module)) {
                ...
                DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_CMDQ_CON, 1 | 0x8000);
...
int _dsi_ps_type_to_bpp(LCM_PS_TYPE ps)
{
    ...
    case LCM_PACKED_PS_18BIT_RGB666:
        return 3;
    case LCM_PACKED_PS_8BIT_SPR:
        return 2;
    case LCM_LOOSELY_PS_8BIT_SPR:
        return 3;
    case LCM_LOOSELY_PS_10BIT_SPR:
        return 3;
    case LCM_PACKED_PS_12BIT_SPR:
        return 2;
    }
    return 0;
}

int _dsi_ps_type_to_ps_sel(LCM_PS_TYPE ps)
{
    switch (ps) {
    case LCM_PACKED_PS_16BIT_RGB565:
        return 0;
                                 ...
                                 int w, int h)
{
    ...
    unsigned int val = 0;
    struct lcm_spr_config_params *spr_params = &dsi_params->spr_params;
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        ...
        if (dsi_params->dsc_enable)
            ...
            ps_sel_bitvalue = _dsi_ps_type_to_ps_sel(dsi_params->PS);

        DISP_REG_SET_FIELD(cmdq, DSI_PSCON_FLD_DSI_PS_SEL, DSI_REG_BASE[i] + DISP_REG_DSI_PSCON,
                            ps_sel_bitvalue);
        if (dsi_params->spr_enable && spr_params->relay == 0) {
            DISP_REG_SET_FIELD(cmdq, DSI_PSCON_FLD_RG_XY_SWAP, DSI_REG_BASE[i] + DISP_REG_DSI_PSCON,
                            spr_params->rg_xy_swap);
            DISP_REG_SET_FIELD(cmdq, DSI_PSCON_FLD_CUSTOM_HEADER_EN,
                            DSI_REG_BASE[i] + DISP_REG_DSI_PSCON,
                            spr_params->custom_header_en);
            DISP_REG_SET_FIELD(cmdq, DSI_PSCON_FLD_CUSTOM_HEADER,
                            DSI_REG_BASE[i] + DISP_REG_DSI_PSCON,
...
enum DSI_STATUS DSI_tx_buf_rw(enum DISP_MODULE_ENUM module, void *cmdq,
                                 LCM_DSI_PARAMS *dsi_params)
{
    unsigned int i = 0;
    unsigned int mmsys_clk = 208;
    unsigned int tmp = 0, rw_times;
    unsigned int preultra_hi, preultra_lo, ultra_hi, ultra_lo, urgent_hi, urgent_lo;
    unsigned int fill_rate, sodi_hi, sodi_lo;
    unsigned int data_rate = dsi_params->data_rate != 0 ?
                             dsi_params->data_rate : dsi_params->PLL_CLOCK * 2;
    unsigned int width = dsi_params->horizontal_active_pixel;
    unsigned int height = dsi_params->vertical_active_line;
    unsigned int dsi_buf_bpp = 0, ps_wc = 0, every_line = 1;

    if (dsi_params->dsc_enable) {
        ps_wc = ((dsi_params->dsc_params.chunk_size + 2) / 3) * 3;
        if (dsi_params->dsc_params.slice_mode == 1)
            ps_wc *= 2;
        dsi_buf_bpp = 3;
    } else
        dsi_buf_bpp = _dsi_ps_type_to_bpp(dsi_params->PS);

    if (!dsi_params->clk_lp_per_line_enable &&
        dsi_params->mode == CMD_MODE)
        every_line = 0;

    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        if (dsi_params->mode == CMD_MODE && every_line != 0) {
            if (dsi_params->dsc_enable) {
                if ((ps_wc % 9) == 0)
        ...
        if (dsi_params->mode == CMD_MODE) {
            if (_dsi_context[i].dsi_params.IsCphy)
                tmp = 25 * data_rate * 2 * dsi_params->LANE_NUM / 7 / 18;
            else
                tmp = 25 * data_rate * dsi_params->LANE_NUM / 8 / 18;
        }

        dprintf(0, "%s, mode=0x%x, tmp=0x%x, ps=%d, width=%d, height=%d, rw_times=%d\n",
                  __func__, dsi_params->mode, tmp, ps_wc, width, height, rw_times);

        /*DISP_REG_SET_FIELD(cmdq, DSI_COM_CON_FLD_DSI_WAIT_DATA_EN,
         *                 DSI_REG_BASE[i] + DISP_REG_DSI_COM_CON, 1);
         */
        DISP_REG_SET_FIELD(cmdq, BUF_BUF_OUTPUT_VALID_THESHOLD,
                           DSI_REG_BASE[i] + DISP_REG_DSI_BUF_CON1, tmp);

        /* send ultra signal between SOF to VACT*/
        DISP_REG_SET_FIELD(cmdq, DSI_VDE_BLOCK_ULTRA,
                           DSI_REG_BASE[i] + DISP_REG_DSI_RESERVED, 0);

        fill_rate = mmsys_clk * 3 / 18;
        tmp = readl(DISP_DSI0_BASE + DISP_REG_DSI_BUF_CON1) >> 16;

        if (_dsi_context[i].dsi_params.IsCphy) {
            sodi_hi = tmp - (12 * (fill_rate - data_rate * 2 * dsi_params->LANE_NUM / 7 / 18) / 10);
            sodi_lo = (23 + 5) * data_rate * 2 * dsi_params->LANE_NUM / 7 / 18;
            preultra_hi = 26 * data_rate * 2 * dsi_params->LANE_NUM / 7 / 18;
            preultra_lo = 25 * data_rate * 2 * dsi_params->LANE_NUM / 7 / 18;
            ultra_hi = 25 * data_rate * 2 * dsi_params->LANE_NUM / 7 / 18;
            ultra_lo = 23 * data_rate * 2 * dsi_params->LANE_NUM / 7 / 18;
            urgent_hi = 12 * data_rate * 2 * dsi_params->LANE_NUM / 7 / 18;
            urgent_lo = 11 * data_rate * 2 * dsi_params->LANE_NUM / 7 / 18;
        } else {
            sodi_hi = tmp - (12 * (fill_rate - data_rate * dsi_params->LANE_NUM / 8 / 18) / 10);
            sodi_lo = (23 + 5) * data_rate * dsi_params->LANE_NUM / 8 / 18;
            preultra_hi = 26 * data_rate * dsi_params->LANE_NUM / 8 / 18;
            preultra_lo = 25 * data_rate * dsi_params->LANE_NUM / 8 / 18;
            ultra_hi = 25 * data_rate * dsi_params->LANE_NUM / 8 / 18;
            ultra_lo = 23 * data_rate * dsi_params->LANE_NUM / 8 / 18;
            urgent_hi = 12 * data_rate * dsi_params->LANE_NUM / 8 / 18;
            urgent_lo = 11 * data_rate * dsi_params->LANE_NUM / 8 / 18;
        }

        DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BUF_SODI_HIGH, sodi_hi);
        DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BUF_SODI_LOW, sodi_lo);
        DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BUF_PREULTRA_HIGH, preultra_hi);
        DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BUF_PREULTRA_LOW, preultra_lo);
        DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BUF_ULTRA_HIGH, ultra_hi);
        DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BUF_ULTRA_LOW, ultra_lo);
        DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BUF_URGENT_HIGH, urgent_hi);
        DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BUF_URGENT_LOW, urgent_lo);

        /* mmsys0 enable urgent signal from dsi buffer
         * mmsys1 ignore urgent signal from dsi, since not connected
         */
        DSI_OUTREG32(cmdq, DISP_REG_CONFIG_MMSYS_EMI_REQ_CTRL, 0xdf);
        DSI_OUTREG32(cmdq, DISP1_REG_CONFIG_MMSYS_EMI_REQ_CTRL, 0xff);

        DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_BUF_RW_TIMES, rw_times);
        DISP_REG_SET_FIELD(cmdq, DSI_BUF_CON0_FLD_EN,
                           DSI_REG_BASE[i] + DISP_REG_DSI_BUF_CON0, 1);
        DISP_REG_SET_FIELD(cmdq, DSI_DEBUG_SEL_FLD_DSI_MM_RST_SEL,
                           DSI_REG_BASE[i] + DISP_REG_DSI_DEBUG_SEL, 1);
    }
...
void DSI_CPHY_clk_setting(enum DISP_MODULE_ENUM module, void *cmdq, LCM_DSI_PARAMS *dsi_params)
{
    ...
    unsigned int pad_phy_lan_0 = 0, pad_phy_lan_1 = 0, pad_phy_lan_2 = 0;
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        ...
        if (dsi_params->lane_swap_en) {
            ...
            pad_phy_lan_0 = (unsigned int)dsi_params->lane_swap[i][MIPITX_PHY_LANE_0];
            pad_phy_lan_1 = (unsigned int)dsi_params->lane_swap[i][MIPITX_PHY_LANE_1];
            pad_phy_lan_2 = (unsigned int)dsi_params->lane_swap[i][MIPITX_PHY_LANE_2];

            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PHY_SEL0, FLD_MIPI_TX_PHY0_SEL,
                pad_mapping[pad_phy_lan_0]);
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PHY_SEL0, FLD_MIPI_TX_PHY1AB_SEL,
                pad_mapping[pad_phy_lan_0]+1);

            /* ENABLE CPHY*/
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PHY_SEL1, FLD_MIPI_TX_CPHY_EN, 0x1);
            ...
            MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL0, 0x76543210);
            MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL1, 0xA4210098);
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        ...
        if (data_Rate != 0) {
            unsigned int tmp = 0, fbksel = 0;

            DISPCHECK("mipitx Data Rate = %d\n", data_Rate);
            if (data_Rate >= 8000) {
                DISPERR("mipitx Data Rate exceed limitation(%d)\n", data_Rate);
                ASSERT(0);
            } else if (data_Rate >= 6000) { /* 6G ~ 8G */
                pcw_ratio = 1;
                posdiv      = 0;
                div3      = 1;
                div3_en      = 0;
                //prediv      = 0;
            } else if (data_Rate >= 3000) { /* 3G ~ 6G */
                pcw_ratio = 2;
                posdiv      = 1;
                div3      = 1;
                div3_en      = 0;
                //prediv      = 0;
            } else if (data_Rate >= 2000) { /* 3G ~ 6G */
                pcw_ratio = 1;
                posdiv      = 0;
                div3      = 3;
                div3_en      = 1;
                //prediv      = 0;
            } else if (data_Rate >= 1500) { /* 1.5G ~ 3G */
                pcw_ratio = 4;
                posdiv      = 2;
                div3      = 1;
                div3_en      = 0;
                //prediv      = 0;
            } else if (data_Rate >= 1000) { /* 1.5G ~ 3G */
                pcw_ratio = 2;
                posdiv      = 1;
                div3      = 3;
                div3_en      = 1;
                //prediv      = 0;
            } else if (data_Rate > 750) { /* 750M ~ 1.5G */
                pcw_ratio = 8;
                posdiv      = 3;
                div3      = 1;
                div3_en      = 0;
                //prediv      = 0;
            } else if (data_Rate > 500) { /* 750M ~ 1.5G */
                pcw_ratio = 4;
                posdiv      = 2;
                div3      = 3;
                div3_en      = 1;
                //prediv      = 0;
            } else if (data_Rate >= 450) { /* 430M ~ 750M */
                pcw_ratio = 16;
                posdiv      = 4;
                div3      = 1;
                div3_en      = 0;
            ...
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PLL_CON1, FLD_RG_DSI_PLL_DIV3_EN, div3_en);
        }
        /* step 4 */
        /* PLL EN */
        spin(1000); /* 30ns */
...
void DSI_DPHY_clk_setting(enum DISP_MODULE_ENUM module, void *cmdq, LCM_DSI_PARAMS *dsi_params)
{
    ...
    unsigned int pad_phy_lan_0 = 0, pad_phy_lan_1 = 0, pad_phy_lan_2 = 0;
    unsigned int pad_phy_lan_3 = 0, pad_phy_lan_ck = 0, pad_phy_lan_rx = 0;
#ifdef LK_FILL_MIPI_IMPEDANCE
    const struct mipitx_impedance m_mipitx_impedance[3] = {
        /* fill mipitx impedance MSB->LSB */
        {5, 5, 0x11C10190, 27},
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        ...
        DISPCHECK("MIPITX (dsi_params->lane_swap_en)=%d\n", (dsi_params->lane_swap_en));
        if (dsi_params->lane_swap_en) {
            DISPCHECK("MIPITX Lane Swap Enabled for DSI Port %d\n", i);
            DISPCHECK("MIPITX Lane Swap mapping: %d|%d|%d|%d|%d|%d\n",
                dsi_params->lane_swap[i][MIPITX_PHY_LANE_0],
            ...
            pad_phy_lan_1 = (unsigned int)dsi_params->lane_swap[i][MIPITX_PHY_LANE_1];
            pad_phy_lan_2 = (unsigned int)dsi_params->lane_swap[i][MIPITX_PHY_LANE_2];
            pad_phy_lan_3 = (unsigned int)dsi_params->lane_swap[i][MIPITX_PHY_LANE_3];
            pad_phy_lan_ck = (unsigned int)dsi_params->lane_swap[i][MIPITX_PHY_LANE_CK];
            pad_phy_lan_rx = (unsigned int)dsi_params->lane_swap[i][MIPITX_PHY_LANE_RX];
            ...
            /* LANE_3 */
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PHY_SEL1, FLD_MIPI_TX_PHY3_SEL,
                pad_mapping[pad_phy_lan_3]);
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PHY_SEL1, FLD_MIPI_TX_CPHYXXX_SEL,
                pad_mapping[pad_phy_lan_3]+1);

            /* CK LANE */
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PHY_SEL0, FLD_MIPI_TX_PHYC_SEL,
                pad_mapping[pad_phy_lan_ck]);
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PHY_SEL0, FLD_MIPI_TX_CPHY1CA_SEL,
                pad_mapping[pad_phy_lan_ck]+1);

            /* LPRX SETTING */
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PHY_SEL1, FLD_MIPI_TX_LPRX0AB_SEL,
                 pad_mapping[pad_phy_lan_rx]);
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PHY_SEL1, FLD_MIPI_TX_LPRX0BC_SEL,
                 pad_mapping[pad_phy_lan_rx]+1);
            ...
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_CK_CKMODE_EN, FLD_DSI_CK_CKMODE_EN, 1);
            MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL0, 0x76543210);
            MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL1, 0x24232098);
            MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL2, 0x76543210);
            MIPITX_OUTREG32(DSI_PHY_REG[i]+MIPITX_PHY_SEL3, 0x00000008);
        }
    }
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
...
#endif
        if (data_Rate < 2500)
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_VOLTAGE_SEL, FLD_RG_DSI_PRD_REF_SEL, 0x0);
        else
            MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_VOLTAGE_SEL, FLD_RG_DSI_PRD_REF_SEL, 0x4);
        ...
        if (data_Rate != 0) {
            ...
            if (data_Rate >= 8000) {
                ...
                div3_en      = 0;
                //prediv      = 0;
            } else if (data_Rate >= 2000) { /* 2G ~ 3G */
                pcw_ratio = 1;
                posdiv      = 0;
                div3      = 3;
                div3_en      = 1;
                //prediv      = 0;
            } else if (data_Rate >= 1500) { /* 1.5G ~ 2G */
                pcw_ratio = 4;
                posdiv      = 2;
                div3      = 1;
                div3_en      = 0;
                //prediv      = 0;
            } else if (data_Rate >= 1000) { /* 1.0G ~ 1.5G */
                pcw_ratio = 2;
                posdiv      = 1;
                div3      = 3;
                div3_en      = 1;
                //prediv      = 0;
            } else if (data_Rate >= 750) { /* 750M ~ 1.0G */
                pcw_ratio = 8;
                posdiv      = 3;
                div3      = 1;
                div3_en      = 0;
                //prediv      = 0;
            } else if (data_Rate >= 510) { /* 510M ~ 750G */
                pcw_ratio = 4;
                posdiv      = 2;
                div3      = 3;
                div3_en      = 1;
        ...
        MIPITX_OUTREGBIT(DSI_PHY_REG[i]+MIPITX_PLL_CON1, FLD_RG_DSI_PLL_FBSEL, (fbksel - 1));
    ...
    DISPCHECK("mipitx MIPITX_PHY_SEL0 = 0x%08x, SEL1 = 0x%08x, SEL2 = 0x%08x, SEL3 = 0x%08x\n",
        readl(DSI_PHY_REG[0]+MIPITX_PHY_SEL0), readl(DSI_PHY_REG[0]+MIPITX_PHY_SEL1),
        readl(DSI_PHY_REG[0]+MIPITX_PHY_SEL2), readl(DSI_PHY_REG[0]+MIPITX_PHY_SEL3));
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
...
void DSI_CPHY_TIMCONFIG(enum DISP_MODULE_ENUM module, void *cmdq, LCM_DSI_PARAMS *dsi_params)
{
    ...
    if (dsi_params->PLL_CLOCK != 0) {
        ui = 1000 / (dsi_params->PLL_CLOCK * 2) > 0 ? 1000 / (dsi_params->PLL_CLOCK * 2) : 1;
        cycle_time = 7000 / (dsi_params->PLL_CLOCK * 2);
    ...
    timcon0.LPX = (dsi_params->LPX == 0) ?
        (NS_TO_CYCLE(75, cycle_time) + 1) : dsi_params->LPX;
    /* spec.  38ns < hs_prpr < 95ns */
    timcon0.HS_PRPR = NS_TO_CYCLE(64, cycle_time) + 1;
    /* spec.  7ui < hs_zero(prebegin) < 448ui */
    timcon0.HS_ZERO = NS_TO_CYCLE((336 * ui), cycle_time);
    /* spec.  7ui < hs_trail(post) < 224ui */
    timcon0.HS_TRAIL = NS_TO_CYCLE((203 * ui), cycle_time);

    /* spec. ta_get = 5*lpx */
    timcon1.TA_GET = (dsi_params->TA_GET == 0) ?
        (5 * timcon0.LPX) : dsi_params->TA_GET;
    /* spec. ta_sure = 1.5*lpx */
    timcon1.TA_SURE = (dsi_params->TA_SURE == 0) ?
        (3 * timcon0.LPX / 2) : dsi_params->TA_SURE;
    /* spec. ta_go = 4*lpx */
    timcon1.TA_GO = (dsi_params->TA_GO == 0) ?
        (4 * timcon0.LPX) : dsi_params->TA_GO;
    /* spec. da_hs_exit > 100ns */
    timcon1.DA_HS_EXIT  = (dsi_params->DA_HS_EXIT == 0) ?
        (NS_TO_CYCLE(125, cycle_time) + 1) : dsi_params->DA_HS_EXIT;

    timcon0.LPX = (timcon0.LPX % 2) ? timcon0.LPX + 1 : timcon0.LPX;  //lpx must be even
    timcon0.HS_PRPR = (timcon0.HS_PRPR % 2) ?
        timcon0.HS_PRPR + 1 : timcon0.HS_PRPR; //hs_prpr must be even
    timcon0.HS_PRPR = timcon0.HS_PRPR >= 6 ? timcon0.HS_PRPR : 6; //hs_prpr must be more than 6
    timcon1.DA_HS_EXIT = (timcon1.DA_HS_EXIT % 2) ?
        timcon1.DA_HS_EXIT : timcon1.DA_HS_EXIT + 1; //da_hs_exit must be odd

    DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI",
        "[DISP] - LK - DSI_PHY_TIMCONFIG, HS_TRAIL = %d, HS_ZERO = %d, HS_PRPR = %d, LPX = %d, TA_GET = %d, TA_SURE = %d, TA_GO = %d\n",
        timcon0.HS_TRAIL, timcon0.HS_ZERO, timcon0.HS_PRPR, timcon0.LPX, timcon1.TA_GET,
        timcon1.TA_SURE, timcon1.TA_GO);
...
void DSI_DPHY_TIMCONFIG(enum DISP_MODULE_ENUM module, void *cmdq, LCM_DSI_PARAMS *dsi_params)
{
    ...
    ui = (1000 / dsi_params->data_rate > 0) ? 1000 / dsi_params->data_rate : 1;
    ...
    hs_trail = (65 * dsi_params->data_rate + 4000) / 8000 + 5;
    ...
    /* spec. clk_trail > 60ns */
    clk_trail = (80 * dsi_params->data_rate + 5000) / 8000 + 5;
    da_hs_sync = 1;
    cont_det = 3;
...
CONFIG_REG:
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        value = REG_FLD_VAL(DSI_PHY_TIMCON0_FLD_LPX, lpx)
            | REG_FLD_VAL(DSI_PHY_TIMCON0_FLD_DA_HS_PREP, hs_prpr)
            | REG_FLD_VAL(DSI_PHY_TIMCON0_FLD_DA_HS_ZERO, hs_zero)
            | REG_FLD_VAL(DSI_PHY_TIMCON0_FLD_DA_HS_TRAIL, hs_trail);
        DSI_OUTREG32(NULL, &DSI_REG[i]->DSI_PHY_TIMECON0, value);

        value = REG_FLD_VAL(DSI_PHY_TIMCON1_FLD_TA_GO, ta_go)
            | REG_FLD_VAL(DSI_PHY_TIMCON1_FLD_TA_SURE, ta_sure)
            | REG_FLD_VAL(DSI_PHY_TIMCON1_FLD_TA_GET, ta_get)
            | REG_FLD_VAL(DSI_PHY_TIMCON1_FLD_DA_HS_EXIT, da_hs_exit);
        DSI_OUTREG32(NULL, &DSI_REG[i]->DSI_PHY_TIMECON1, value);

        value = REG_FLD_VAL(DSI_PHY_TIMCON2_FLD_CONT_DET, cont_det)
            | REG_FLD_VAL(DSI_PHY_TIMCON2_FLD_DA_HS_SYNC, da_hs_sync)
            | REG_FLD_VAL(DSI_PHY_TIMCON2_FLD_CLK_HS_ZERO, clk_zero)
            | REG_FLD_VAL(DSI_PHY_TIMCON2_FLD_CLK_HS_TRAIL, clk_trail);
        DSI_OUTREG32(NULL, &DSI_REG[i]->DSI_PHY_TIMECON2, value);

        value = REG_FLD_VAL(DSI_PHY_TIMCON3_FLD_CLK_HS_PREP, clk_hs_prpr)
            | REG_FLD_VAL(DSI_PHY_TIMCON3_FLD_CLK_HS_POST, clk_hs_post)
            | REG_FLD_VAL(DSI_PHY_TIMCON3_FLD_CLK_HS_EXIT, clk_hs_exit);
...
/* LK does not rely on INTEN, use polling to handle event  */
            ...
            DSI_OUTREG32(cmdq, DSI_REG_BASE[d] + DISP_REG_DSI_CMDQ_CON, 2 | 0x8000);
        ...
        if (DSI_REG[d]->DSI_MODE_CTRL.MODE != 0) {
            ...
            if (cmd < 0xB0) {
                if (count > 1) {
                    ...
                    DSI_OUTREG32(cmdq, DSI_REG_BASE[d] + DISP_REG_DSI_CMDQ_CON,
                                                                (2+(count)/4) | 0x8000);
                } else {
                    t0.CONFG = 0;
                    t0.Data0 = cmd;
                    ...
                    DSI_OUTREG32(cmdq, &DSI_CMDQ_REG[d]->data[0], AS_UINT32(temp));
                    DSI_OUTREG32(cmdq, DSI_REG_BASE[d] + DISP_REG_DSI_CMDQ_CON, 1 | 0x8000);
                }
            } else {
                if (count > 1) {
                    t2.CONFG = 2;
                    ...
                    unsigned int queue_size, unsigned char force_update)
{
    ...
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        if (DSI_REG[i]->DSI_MODE_CTRL.MODE != 0) {
            ...
            DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_CMDQ_CON, queue_size | 0x8000);

            for (j = 0; j < queue_size; j++)
                DISPMSG("[DISP] - kernel - %s. DSI_CMDQ+%04x : 0x%08x\n", __func__,
                            j*4, readl(DISP_DSI0_BASE + 0xd00 + j*4));
                    ...
                    void *cmdq_handle)
{
   ...
   DSI_tx_buf_rw(module, NULL, &(config->dsi_config));
   DSI_PHY_TIMCONFIG(module, NULL, &(config->dsi_config));
    ...
    // Set "DSI CPHY CMD Type 2 6-Byte Enable" as 0
    for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
        if (_dsi_context[i].dsi_params.IsCphy)
            DISP_REG_SET_FIELD(cmdq_handle, DSI_CMD_CPHY_6BYTE_EN,
                                DSI_REG_BASE[0] + DISP_REG_DSI_CMD_TYPE1_HS, 0);
    }
...
int ddp_dsi_trigger(enum DISP_MODULE_ENUM module, void *cmdq)
{
...
#if 0
    //dsi pattern
    DSI_BIST_Pattern_Test(module, NULL, 1, 0x11);
    DSI_DumpRegisters(module, 1);
    ...
    if (_dsi_context[i].dsi_params.mode == CMD_MODE) {
        ...
        if (!_dsi_context[i].dsi_params.clk_lp_per_line_enable) {
            DISP_REG_SET_FIELD(cmdq, DSI_COM_CON_FLD_DSI_WAIT_DATA_EN,
                           DSI_REG_BASE[i] + DISP_REG_DSI_COM_CON, 0);
            DISP_REG_SET_FIELD(cmdq, DSI_CMD_HS_BLANKING_EN,
                           DSI_REG_BASE[i] + DISP_REG_DSI_CMD_TYPE1_HS, 1);
        }
    ...
    if (!_dsi_context[i].dsi_params.clk_lp_per_line_enable &&
        ...
        DSI0_WaitNotBusyAfterTrigger(module, false);
        DISP_REG_SET_FIELD(cmdq, DSI_CMD_HS_BLANKING_EN,
                       DSI_REG_BASE[i] + DISP_REG_DSI_CMD_TYPE1_HS, 0);
    }
    return 0;
}
