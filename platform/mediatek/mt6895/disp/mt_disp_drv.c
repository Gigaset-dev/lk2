...
static void display_pl_boottag_hook(struct boot_tag *tag)
{
    ...
    if (boot_tag_temp != NULL) {
       display_info.islcmfound = boot_tag_temp->islcmfound;
       strcpy(display_info.lcm_name, boot_tag_temp->lcm_name);
    } else {
        dprintf(INFO, "boot display info is NULL\n");
    }
}
...
static void mt_disp_init(void *lcd_pa_base, void *lcd_va_base, void *fdt)
{
    ...
    dprintf(CRITICAL, "fb_va: 0x%llx, fb_pa: 0x%08x, fb_pa_k: 0x%llx\n",
    ...
    dprintf(CRITICAL, "%s %d, va=0x%lx, found:%d, lcm:\"%s\"\n",
...
void disp_lcm_fill_lcm_dts_phandle(void *fdt)
{
    ...
    return;
#else
    int nodeoffset, data, phandle, nodeoffset1;
    int idx;
    const void *val, *val1;
    char buf[100];
    const char *phy_type = "mediatek,mt6895-mipi-tx-cphy";

#ifndef DISP_PRIM_DUAL_PIPE
...
error:
    dprintf(CRITICAL, "set dsi panel index to dtb failed.\n");
...
error2:
    dprintf(CRITICAL, "set mipi tx to dtb failed.\n");
#endif
}
