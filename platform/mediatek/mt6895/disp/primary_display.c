...
#if defined(DISP_PRIM_DUAL_PIPE)
    if (lcm_param != NULL) {
        if (lcm_param->dsi.output_mode == MTK_PANEL_DUAL_PORT ||
                (lcm_param->dsi.dsc_enable == 1 &&
                lcm_param->dsi.dsc_params.slice_mode == 1))
            scenario = DDP_SCENARIO_PRIMARY_DISP_LEFT;
    }
#endif
...
int primary_display_trigger(int blocking)
{
    ...
    if (pgc->mode == DIRECT_LINK_MODE) {
        if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
            if (primary_display_is_video_mode())
                return 0;
            DSI0_WaitNotBusyAfterTrigger(DISP_MODULE_DSI0, force);
        }
        ...
        if (primary_display_use_cmdq == CMDQ_DISABLE) {
            if (!primary_display_is_video_mode())
                DSI0_Wait_TE(DISP_MODULE_DSI0, force);
...
int primary_display_setbacklight(unsigned int level)
{
    ...
    if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
        ...
        mode = readl(DSI_REG_BASE[0] + DISP_REG_DSI_MODE_CON) & 0x00000003;
        if (mode != CMD_MODE) {
            DISP_REG_SET_FIELD(NULL, DSI_START_FLD_DSI_START,
        ...
        if (mode != CMD_MODE) {
            DSI_SetMode(DISP_MODULE_DSI0, NULL, mode);
            DISP_REG_SET_FIELD(NULL, DSI_START_FLD_DSI_START,
                                    DSI_REG_BASE[0] + DISP_REG_DSI_START, 0);
            DISP_REG_SET_FIELD(NULL, DSI_START_FLD_DSI_START,
...
#else
    ALIGN_TO(LCM_MAX_WIDTH, MTK_FB_ALIGNMENT);
#endif
}
