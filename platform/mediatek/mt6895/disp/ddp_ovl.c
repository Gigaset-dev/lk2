...
unsigned long ovl_base_addr(enum DISP_MODULE_ENUM module)
{
    ...
    case DISP1_MODULE_OVL0:
        return DISPSYS1_OVL0_BASE;
    case DISP1_MODULE_OVL0_2L:
        return DISPSYS1_OVL0_2L_BASE;
    case DISP1_MODULE_OVL1_2L:
        return DISPSYS1_OVL1_2L_BASE;
    default:
        return 0;
    }
}
...
unsigned long mmsys_ovl_ultra_reg(enum DISP_MODULE_ENUM module)
{
    switch (module) {
    case DISP_MODULE_OVL0:
    case DISP_MODULE_OVL0_2L:
    case DISP_MODULE_OVL1_2L:
        return DISP_REG_CONFIG_MMSYS_MISC;
    case DISP1_MODULE_OVL0:
    case DISP1_MODULE_OVL0_2L:
    case DISP1_MODULE_OVL1_2L:
        return DISP1_REG_CONFIG_MMSYS_MISC;
    default:
        return 0;
    }
    return 0;
...
#else
    if (module == DISP1_MODULE_OVL0 || module == DISP1_MODULE_OVL0_2L
                                    || module == DISP1_MODULE_OVL1_2L)
        idx = (base - DISPSYS1_OVL0_BASE) / DISP_INDEX_OFFSET;
    else
...
static unsigned int ovl_dispsys_index(enum DISP_MODULE_ENUM module)
{
    int idx = 0;
...
int OVLStart(enum DISP_MODULE_ENUM module, void *handle)
{
    ...
    unsigned int disp_idx = ovl_dispsys_index(module);

#if 0
    //set red background
    DISP_REG_SET(handle, disp_idx * DISPSYS_OVL_OFFSET + idx * DISP_INDEX_OFFSET +
               DISP_REG_OVL_ROI_BGCLR, 0xFFFF0000);
#endif
    DISP_REG_SET_FIELD(handle, EN_FLD_OVL_EN,
               disp_idx * DISPSYS_OVL_OFFSET + idx * DISP_INDEX_OFFSET +
               DISP_REG_OVL_EN, 0x1);
...
int OVLStop(enum DISP_MODULE_ENUM module, void *handle)
{
    ...
    DISP_REG_SET(handle, disp_idx * DISPSYS_OVL_OFFSET +
        idx * DISP_INDEX_OFFSET+DISP_REG_OVL_INTEN, 0x00);
    DISP_REG_SET(handle, disp_idx * DISPSYS_OVL_OFFSET +
        idx * DISP_INDEX_OFFSET+DISP_REG_OVL_EN, 0x00);
    DISP_REG_SET(handle, disp_idx * DISPSYS_OVL_OFFSET +
        idx * DISP_INDEX_OFFSET+DISP_REG_OVL_INTSTA, 0x00);
...
#define OVL_IDLE (0x3)
    ...
    DISP_CPU_REG_SET(disp_idx * DISPSYS_OVL_OFFSET +
            idx * DISP_INDEX_OFFSET+DISP_REG_OVL_RST, 0x1);              // soft reset
    DISP_CPU_REG_SET(disp_idx * DISPSYS_OVL_OFFSET +
            idx * DISP_INDEX_OFFSET+DISP_REG_OVL_RST, 0x0);
    while (!(DISP_REG_GET(disp_idx * DISPSYS_OVL_OFFSET +
            idx * DISP_INDEX_OFFSET+DISP_REG_OVL_FLOW_CTRL_DBG) & OVL_IDLE)) {
        delay_cnt++;
        spin(10);
        if (delay_cnt > 2000) {
           ...
           void *handle)
{
    ...
    DISP_REG_SET(handle, disp_idx * DISPSYS_OVL_OFFSET +
        idx * DISP_INDEX_OFFSET+DISP_REG_OVL_ROI_SIZE, bgH<<16 | bgW);

    DISP_REG_SET(handle, disp_idx * DISPSYS_OVL_OFFSET +
        idx * DISP_INDEX_OFFSET+DISP_REG_OVL_ROI_BGCLR, bgColor);
                   ...
                   void *handle)
{
    ...
    unsigned int addr = disp_idx * DISPSYS_OVL_OFFSET + idx * DISP_INDEX_OFFSET +
                   ...
                   void *handle)
{
    ...
    unsigned int idx_offset  = disp_idx * DISPSYS_OVL_OFFSET + idx * DISP_INDEX_OFFSET;
...
void OVLRegStore(enum DISP_MODULE_ENUM module)
{
    ...
    DDPDBG("store %c cnt registers on ovl %u_%u", reg_back_cnt[idx], idx, disp_idx);
}
...
void OVLRegRestore(enum DISP_MODULE_ENUM module, void *handle)
{
    ...
    if (idx >= OVL_NUM) {
        DDPERR("[%s]ovl_idx value is error: %d\n", __func__, idx);
        return;
    }
    ...
    DDPDBG("restore %d cnt registers on ovl %u_%u", reg_back_cnt[idx], idx, disp_idx);
    reg_back_cnt[idx] = 0;
}
...
int  OVLClockOn(enum DISP_MODULE_ENUM module, void *handle)
{
    ...
    DDPMSG("OVL%u_%u ClockOn CG 0x%x\n", idx, disp_idx,
...
static inline int is_module_ovl(enum DISP_MODULE_ENUM module)
{
    if (module == DISP_MODULE_OVL0 ||
        ...
        module == DISP1_MODULE_OVL0 ||
        module == DISP1_MODULE_OVL0_2L ||
        module == DISP1_MODULE_OVL1_2L)
        return 1;
    else
        return 0;
        ...
        enum DISP_MODULE_ENUM next, int connect, void *handle)
{
    ...
    if (connect && is_module_ovl(next)) {
        ...
        case DISP_MODULE_OVL1_2L:
            ...
            break;
        case DISP1_MODULE_OVL0:
            DISP_REG_SET_FIELD(handle, FLD_CON_OVL0, DISP1_REG_CONFIG_MMSYS_OVL_CON, 0x2);
            break;
        case DISP1_MODULE_OVL0_2L:
            DISP_REG_SET_FIELD(handle, FLD_CON_OVL0_2L, DISP1_REG_CONFIG_MMSYS_OVL_CON, 0x2);
            break;
        case DISP1_MODULE_OVL1_2L:
            DISP_REG_SET_FIELD(handle, FLD_CON_OVL1_2L, DISP1_REG_CONFIG_MMSYS_OVL_CON, 0x2);
            break;
        default:
            DDPERR("invalid module=%d\n", module);// invalid module
            ASSERT(0);
        ...
        case DISP_MODULE_OVL1_2L:
            ...
            break;
        case DISP1_MODULE_OVL0:
            DISP_REG_SET_FIELD(handle, FLD_CON_OVL0, DISP1_REG_CONFIG_MMSYS_OVL_CON, 0x1);
            break;
        case DISP1_MODULE_OVL0_2L:
            DISP_REG_SET_FIELD(handle, FLD_CON_OVL0_2L, DISP1_REG_CONFIG_MMSYS_OVL_CON, 0x1);
            break;
        case DISP1_MODULE_OVL1_2L:
            DISP_REG_SET_FIELD(handle, FLD_CON_OVL1_2L, DISP1_REG_CONFIG_MMSYS_OVL_CON, 0x1);
            break;
        default:
            DDPERR("invalid module=%d\n", module);// invalid module
            ASSERT(0);
...
void OVLDump(enum DISP_MODULE_ENUM module)
{
    ...
    module_base = disp_idx * DISPSYS_OVL_OFFSET + idx * DISP_INDEX_OFFSET + DISP_OVL0_OVL_STA;
...
void ovl_dump_analysis(enum DISP_MODULE_ENUM module)
{
    ...
    unsigned int idx = ovl_index(module);
    unsigned int disp_idx = ovl_dispsys_index(module);
    unsigned int offset = disp_idx * DISPSYS_OVL_OFFSET + idx * DISP_INDEX_OFFSET;
...
static int ovl_golden_setting(enum DISP_MODULE_ENUM module, void *cmdq)
{
    ...
    /* enable ultra signal from rdma to ovl */
    DISP_REG_SET_FIELD(cmdq, mmsys_ovl_ultra_offset(module),
       mmsys_ovl_ultra_reg(module), 0);
...
//        ovl_layer_config(module, local_layer, has_sec_layer, ovl_cfg, handle);
    ...
    DISP_REG_SET(handle, disp_idx * DISPSYS_OVL_OFFSET + idx * DISP_INDEX_OFFSET +
        DISP_REG_OVL_SRC_CON, enabled_layers);
