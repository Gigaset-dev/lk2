                ...
                if (lcm_drv->compare_id(fdt) != 0) {
                    ...
                    if (target_fps > 0) {
                        if (hlcm_param.dsi.fps != target_fps) {
                            DISPCHECK("lcm:%s found with fps:%u didn't match target fps:%u\n",
                                lcm_drv->name, hlcm_param.dsi.fps, target_fps);
                            _display_interface_path_deinit(handle);
                            continue;
                        } else {
                            DISPCHECK("lcm:%s found with fps:%u, target fps:%u\n",
                                lcm_drv->name, hlcm_param.dsi.fps, target_fps);
                        }
                    }
#endif
