...
static const char *ddp_greq_name0(int bit)
{
    switch (bit) {
    case 0:
...
static const char *ddp_greq_name1(int bit)
{
    switch (bit) {
    case 0:
        return "DISP_OVL0_SYSRAM_SLB";
    case 1:
        return "DISP_OVL0_2L_SYSRAM_SLB";
    case 2:
...
static void mmsys_config_dump_reg(int mmsys_id)
{
    ...
    DDPDUMP("MMSYS_CONFIG: 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x, 0x%04x=0x%08x\n",
        ...
        0xFE8, readl(module_base + 0xFE8),
        0xFEC, readl(module_base + 0xFEC));
...
static void mmsys_config_dump_analysis(int mmsys_id)
{
    ...
    if (mmsys_id == 0) {
        ...
        greq0 = DISP_REG_GET(DISP1_REG_CONFIG_SMI_LARB0_GREQ);
        greq1 = DISP_REG_GET(DISP1_REG_CONFIG_SMI_LARB1_GREQ);
    }
    ...
    reg = (mmsys_id == 0) ? DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON1) :
            DISP_REG_GET(DISP1_REG_CONFIG_MMSYS_CG_CON1);
    for (i = 0; i < 32; i++) {
        if ((reg & (1 << i)) == 0) {
            name = ddp_clock_1(i);
            if (name)
