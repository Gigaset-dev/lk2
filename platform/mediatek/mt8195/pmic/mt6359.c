...
{
    for (int i = 0; i < countof(init_setting); i++)
        pwrap_write_field(init_setting[i].addr, init_setting[i].val,
                          init_setting[i].mask, init_setting[i].shift);
}

static void pmic_lp_setting(void)
{
    for (int i = 0; i < countof(lp_setting); i++)
        pwrap_write_field(lp_setting[i].addr, lp_setting[i].val,
