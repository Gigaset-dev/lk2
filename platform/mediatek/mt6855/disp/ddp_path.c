...
int primary_disp_dsc_module_list[DDP_ENING_NUM] = {
    DISP_MODULE_OVL1_2L, DISP_MODULE_OVL0,
    ...
    DISP_MODULE_DITHER0, DISP_MODULE_SPR0_VIRTUAL,
...
static struct mout_t mout_map[] = {
    ...
    {DISP_MODULE_DITHER0,
        {{DISP_MODULE_SPR0_VIRTUAL, 1 << 0},
         {-1, 0} },
        0, 0},

    /* DISP_SPR0_MOUT_EN */
    {DISP_MODULE_SPR0_VIRTUAL,
...
int ddp_path_init(void)
{
    ...
    /* mout */
    mout_map[0].reg =
        (unsigned long *)DISP_REG_CONFIG_DISP_OVL1_2L_BLEND_MOUT_EN;
    mout_map[1].reg =
    ...
    /* Set SMI_LARB 0x380 to 0 only for FPGA disable IOMMU */
    for (i = 0; i < 16; i++) {
...
int ddp_path_top_clock_on(void)
{
    ...
    DISP_REG_SET_FIELD(NULL, FLD_CG1_DISP_URGENT, DISP_REG_CONFIG_MMSYS_CG_CLR2, 1);
    DDPMSG("ddp CG0:%x CG1:%x\n",
        DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0),
        DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON1));

#if 0  /* Force CG always on */
    DDPMSG("[FPGA Only] before power on MMSYS CG:0x%x,0x%x,0x%x,MIPI CG:0x%x\n",
           DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON0),
           DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON1),
           DISP_REG_GET(DISP_REG_CONFIG_MMSYS_CG_CON2),
           DISP_REG_GET(DISP_REG_APMIXED_PLL_CON0));
