...
void disp_init(void *fdt)
{
    ...
    ret = snprintf(buf_name, 127, "framebuffer");
    if (ret < 0 || ret > 127)
        dprintf(CRITICAL, "snprintf failed of buf name\n");
    if (mboot_params_current_abnormal_boot() > 0) {
        ret = mrdump_get_reserved_info(buf_name, &fb_addr, &fd_size);
        if (ret < 0 || !fb_addr) {
            dprintf(CRITICAL, "query reserved buffer failed, %d\n", ret);
            return;
        } else
            g_fb_base = fb_addr;
    } else {
...
void mt_disp_config_frame_buffer(void *fdt)
{
    ...
    if (fdt_setprop(fdt, offset, "atag,videolfb-fb_base_h", &fb_base_h, sizeof(fb_base_h)))
        DISPERR("%s[%d]fdt_setprop failed\n", __func__, __LINE__);
    if (fdt_setprop(fdt, offset, "atag,videolfb-fb_base_l", &fb_base_l, sizeof(fb_base_l)))
        DISPERR("%s[%d]fdt_setprop failed\n", __func__, __LINE__);
    if (fdt_setprop(fdt, offset, "atag,videolfb-vramSize", &g_fb_size, sizeof(g_fb_size)))
        DISPERR("%s[%d]fdt_setprop failed\n", __func__, __LINE__);
}
...
void target_atag_videolfb(void *fdt)
{
    ...
    *p++ = DISP_GetVRamSize();
    ...
    if (lcmname != NULL)
        ...
        dprintf(CRITICAL, "videolfb - lcmname    = NULL\n");

    offset = fdt_path_offset(fdt, "/chosen");
    if (offset >= 0) {
        if (fdt_setprop(fdt, offset, "atag,videolfb", ptr, (void *)p - (void *)ptr))
            DISPERR("%s[%d]fdt_setprop failed\n", __func__, __LINE__);
    } else
...
void disp_lcm_fill_lcm_dts_phandle(void *fdt)
{
    ...
    const void *val, *val1;
    char buf[100];
    const char *phy_type = "mediatek,mt6855-mipi-tx-cphy";
    ...
    if (!fdt_setprop(fdt, nodeoffset, "remote-endpoint", &phandle, sizeof(phandle))) {
        val = fdt_getprop(fdt, nodeoffset, "remote-endpoint", NULL);
        if (val) {
            memcpy(&data, val, sizeof(data));
            data = fdt32_to_cpu(data);
        ...
        DISPERR("update dsi remote-endport failed when lk setprop\n");
error:
    ...
    if (dts_is_cphy) {
        ...
        if (!fdt_setprop_string(fdt, nodeoffset1, "compatible", phy_type)) {
            val1 = fdt_getprop(fdt, nodeoffset1, "compatible", NULL);
            if (val1)
                DISPMSG("set dsi mipi tx data as %s done\n", (char *)val1);
            else
                DISPERR("set dsi mipi tx data failed\n");
        } else
            DISPERR("set dsi compatible failed when lk setprop string\n");
    }
