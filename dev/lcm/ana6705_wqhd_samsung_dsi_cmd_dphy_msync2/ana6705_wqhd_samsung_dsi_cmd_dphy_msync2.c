...
#define LCM_ID_AMB670YF01 (0xC5) // YF01
#define LCM_ID_AMB670YF01_1 (0xA7) // YF07
...
#define MT6373_PMIC_VMCH_ADDR 0x1CB1
#define MT6373_PMIC_VMCH_MASK 0x1
#define MT6373_PMIC_VMCH_SHIFT 0

#define MT6363_PMIC_VUFS18_ADDR 0x1C3F
#define MT6363_PMIC_VUFS18_MASK 0x1
#define MT6363_PMIC_VUFS18_SHIFT 0

static int mt6373_ldo_3p0_enable(u8 en)
{
    int ret = 0, val = 0;
    struct spmi_device *mt6373_devs; //MT 6373 ID: 0x5

    mt6373_devs = get_spmi_device(SPMI_MASTER_1, SPMI_SLAVE_5);//MT 6373 ID: 0x5
    if (!mt6373_devs) {
        LCM_LOGI("[LCM][ERROR]get mt6373_devs err %s\n", __func__);
        return ERR_INVALID_ARGS;
    }

    ret = spmi_ext_register_writel(mt6373_devs, MT6373_PMIC_VMCH_ADDR, &en, 1);
    ret = spmi_ext_register_readl(mt6373_devs, MT6373_PMIC_VMCH_ADDR, &val, 1);
    LCM_LOGI("%s [LCM]ret= %d val= %d\n", __func__, ret, val);

    return ret;
}

static int mt6363_ldo_1p8_enable(u8 en)
{
    int ret = 0, val = 0;
    struct spmi_device *mt6363_devs; //MT 6363 ID: 0x4

    mt6363_devs = get_spmi_device(SPMI_MASTER_1, SPMI_SLAVE_4);//MT 6363 ID: 0x4
    if (!mt6363_devs) {
        LCM_LOGI("[LCM][ERROR]get mt6363_devs err %s\n", __func__);
        return ERR_INVALID_ARGS;
    }

    ret = spmi_ext_register_writel(mt6363_devs, MT6363_PMIC_VUFS18_ADDR, &en, 1);
    LCM_LOGI("[LCM]%s ret=%d en=%d\n", __func__, ret, en);
    en = 0;//set 1.8V
    ret = spmi_ext_register_writel(mt6363_devs, 0x1f17, &en, 1);
    LCM_LOGI("[LCM]%s ret=%d en=%d\n", __func__, ret, en);
    ret = spmi_ext_register_readl(mt6363_devs, MT6363_PMIC_VUFS18_ADDR, &val, 1);
...
static struct LCM_setting_table init_setting[] = {
    //DSC 10bit
    {0x9E, 89, {0x11, 0x00, 0x00, 0xAB, 0x30, 0x80, 0x0C, 0x90, 0x05,
    ...
    {0xB9, 0x01, {0x02} }, /* TSP_VSYNC Fixed TE 02:120 03:90 05:60 */
    ...
    {0xF2, 0x02, {0x1B, 0x50} },/* 0x1B,0x50:120hz 0x24,0x70:90hz */
    {0xBD, 0x02, {0x21, 0x02} },/* 0x21,0x02:120hz 0x21,0x03:90hz */
    {0x60, 0x01, {0x00} },/* 0x00:120hz 0x08:90hz 0x01:60hz */
    {0xB0, 0x03, {0x00, 0x10, 0xBD} },
    {0xBD, 0x01, {0x10} },/* 0x00:1frame 0x10:2frame 0x20:3frame */
    {0xF7, 0x01, {0x0F} },
    {0xF0, 0x02, {0xA5, 0xA5} },
    /* Common Setting */
    {0xF0, 0x02, {0x5A, 0x5A} },
    {0xF2, 0x02, {0x00, 0x01} },/* FQ_CON Setting */
    {0xF0, 0x02, {0xA5, 0xA5} },
    /* TE(Vsync) ON */
    {0xF0, 0x02, {0x5A, 0x5A} },
    {0x35, 0x01, {0x00} },
    ...
    {0x63, 0x01, {0x01} }, /* Dimming Setting */
    ...
    {0x1F, 0x10, {0x01, 0x17, 0x01, 0x18, 0x06, 0x88, 0x06, 0x89,
                  0x0A, 0xE2, 0x0A, 0xE3, 0x11, 0xF5, 0x1B, 0xC0} },
    {0xB0, 0x03, {0x00, 0x6A, 0x1F} },
    {0x1F, 0x2C, {0x01, 0x17, 0x03, 0x44, 0x04, 0x5A, 0x05, 0x71,
                  0x06, 0x88, 0x07, 0x9E, 0x08, 0xB5, 0x09, 0xCB, 0x0A, 0xE2, 0x0B,
...
{
    MDELAY(10);
    mt_set_gpio_mode(GPIO148, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO148, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO148, 1); //1.5V
    MDELAY(10);
    mt6373_ldo_3p0_enable(1);//3.0
    MDELAY(10);

    mt_set_gpio_mode(GPIO24, GPIO_MODE_00); //TE
    mt_set_gpio_dir(GPIO24, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO24, 1);
    mt_set_gpio_pull_select(GPIO36, GPIO_NO_PULL);
    mt_set_gpio_pull_select(GPIO22, GPIO_NO_PULL);
    MDELAY(10);

    mt_set_gpio_mode(GPIO86, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO86, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO86, GPIO_OUT_ONE);
    MDELAY(5);

    mt_set_gpio_out(GPIO86, GPIO_OUT_ZERO);
    MDELAY(5);
    //SET_RESET_PIN(1);
    mt_set_gpio_out(GPIO86, GPIO_OUT_ONE);
    MDELAY(10);
...
LCM_DRIVER ana6705_wqhd_samsung_dsi_cmd_dphy_msync2_lcm_drv = {
    .name = "ana6705_wqdp_samsung_dsi_cmd_dphy_msync2_lcm_drv",
