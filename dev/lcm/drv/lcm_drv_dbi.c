...
static LCM_UTIL_FUNCS lcm_util_dbi;
        ...
        struct mtk_panel_cust *cust)
{
    ...
    if (ctx_dbi.panel_resource == NULL) {
        LCM_MALLOC(ctx_dbi.panel_resource, sizeof(struct mtk_panel_resource));
        if (ctx_dbi.panel_resource == NULL) {
            LCM_MSG("%s: failed to allocate panel resource\n", __func__);
            return -ENOMEM;
        }
        memset(ctx_dbi.panel_resource, 0, sizeof(struct mtk_panel_resource));
    }

    target_cust = &ctx_dbi.panel_resource->cust;
...
int mtk_lcm_dbi_parse_lcm_dts(const void *fdt, const char *lcm_name)
{
    ...
    ret = mtk_lcm_gateic_drv_init(fdt, MTK_LCM_FUNC_DBI, lcm_name);
    if (ret != 0) {
    ...
    ret = load_panel_resource_from_dts(fdt, offset, ctx_dbi.panel_resource);
    if (ret != 0 || ctx_dbi.panel_resource == NULL) {
...
int mtk_lcm_dbi_parse_next_lcm_dts(const void *fdt, char *lcm_name, bool force)
{
    ...
    if (force == true) {
        id = 0;
        ret = mtk_lcm_list_init(MTK_LCM_FUNC_DBI);
        if (ret <= 0) {
...
retry:
    if (id >= mtk_lcm_get_drv_count(MTK_LCM_FUNC_DBI)) {
        LCM_DBG("%s:no more lcm dts\n", __func__);
        return -ENOMEM;
    }

    ctx_dbi.parsing = 0;
    LCM_FREE(ctx_dbi.panel_resource, sizeof(struct mtk_panel_resource));

    temp = mtk_lcm_get_drv_name(MTK_LCM_FUNC_DBI, id);
    if (temp == NULL) {
        LCM_MSG("%s:failed to get lcm name\n", __func__);
        goto retry;
    }
    LCM_MSG("%s: parse lcm dts:%s, id:%u\n", __func__, temp, id);
    ret = mtk_lcm_dbi_parse_lcm_dts(fdt, temp);
    if (ret != 0) {
...
int mtk_lcm_dbi_delay(char type, unsigned int time)
{
    ...
    case MTK_LCM_UTIL_TYPE_MDELAY:
        if (lcm_util_dbi.mdelay)
            lcm_util_dbi.mdelay(time);
        break;
    case MTK_LCM_UTIL_TYPE_UDELAY:
        if (lcm_util_dbi.udelay)
            lcm_util_dbi.udelay(time);
        break;
    default:
...
LCM_DRIVER mtk_lcm_dbi_drv = {
    .name           = "mediatek,lcm-dbi-common_driver",
};
